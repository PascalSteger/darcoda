...
if investigate == 'sim' or investigate == 'simple':
    ...
    # If positive assume constant; if negative take fraction of local baryonic value for that bin: 
...
checksigma = True         # check sigma_los integration (set nu, M to interpolated data values)
...
model      = True          # for Walker mock data: plot model, TODO: work with it
if investigate != 'walker': # for other investigations: no models yet
    model = False
# TODO: get done together with 'analytic'

nplot      = 1.   # only plot every 16th plot, in a random fashion
...
lim        = False         # adjust range of plots to predetermined values
#log       = False          # yaxis in (M/dens) scaled in log?
even       = 'avg'        # for simps integration (everywhere): 'avg', 'first', 'last'
log = True if plotdens else False

scalemass = False                        # scale mass s.t. Mmax=1
...
mprior = -1                            # Mass prior
deltaprior  = True                     # Deltaprior: - beta (velocity anisotropy) in spherical
...
nutol   = 3.0                     # (nu_(i+1) - nu_i) must be < nutol * nu_(i+1)
ktol    = 3.0                     # same as for nu
...
global nu1_x, nu2_x, sig1_x, sig2_x, M_x
global fnewoverf
...
nuerrcorr = 1.                          # 2.   # TODO: include *2 in readout routine, if real
sigerrcorr = 1.                         # 1.5

densstart = -1.0                        # -2.6 for Hernquist, -1. for Walker
scaledens = 3.
scalepower = 1.5                      # 0.95 for Hernquist, 1.5 for Walker
chisqtol = 70. if (pops == 1) else 60.  # more information in two tracer pops
endcount = 300                  # 300 accepted models which chisq<chisqtol means initialization phase is over

rejcount = 1.                   # Rejection count
acccount = 0.                   # Acceptance count
accrejtollow  = 0.24             # Acceptance/rejection rate
accrejtolhigh = 0.26             #
farinit = 1./7.                # 5 times chisq is too high in init phase: start new from last point
stepafterrunaway = 2.          # divide stepsize by this amount if too low fnewoverf 2.5
