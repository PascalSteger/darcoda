...
def ant_kapr4(r0, intdelta, M, nu, sigr2): # [pc], [1], [munit], [munit/pc^3], [(km/s)^2]
    # kappa^4_r

    # get full integrand first
    igra = np.log(gp.G1 * M * nu * sigr2/r0**2 * np.exp(intdelta)) # [G]*[msun^2/pc^5]*[(km/s)^4]
    ...
    for i in range(gp.nipol):
        ...
        yint *= np.exp(intdelta[i:])    # [munit/pc^4 (km/s)^4]

        # case a: add integrated polynomial above 3r_S only
        tmp[i] = 3.*(np.exp(-intdelta[i])/nu[i]) * \
...
def ant_kaplos4surf(r0, delta, nu, kapr4): # [1], [pc], [1], [munit/pc^3], [(km/s)^4], gives back [(munit/pc^2) (km/s)^4]
    ...
    return np.hstack([tmp,np.exp(r0[-1:]*polyhilo[0]+polyhilo[1])])    # [munit/pc^2 (km/s)^4]
...
def int_2D3D(r, nu2d):                   # [munit/lunit^2], [lunit]
    # TODO: replacement for this crap! need better deprojection in the first place, do not want to miss so much at the end of nu_i
