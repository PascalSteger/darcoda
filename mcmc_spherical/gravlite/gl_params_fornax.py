...
# if poly: denslog = False

densstart = -2.              # -2.6 for Hernquist, -2.3979 for Walker -1.8 fornax
scaledens = 1. # percentage of maximum radius from data, for which the polynomial is scaled
scalepower = 2.5                  # 0.95 for Hernquist, 1.5 for Walker, 2.3 fornax
...
nutol   = 2.0     # (nu_(i+1) - nu_i) must be < nutol * nu_(i+1) (surface density in case of sphere)
ktol    = 0.5      # same as for nu, but for dens in disc case, 50% up is still fine
...
chi2tol = 50. if (pops == 1) else 50.  # more information in two tracer pops, but more errors as well. TODO: reduced chi**2
...
stepafterrunaway = 0.8 # mult. stepsize by this amount if too low fnewoverf 2.5
