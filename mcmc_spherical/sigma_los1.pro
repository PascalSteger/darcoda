...
FUNCTION sigma_los1, r_in, rp_nu, rp_M, rp_beta, Mpars, nupars, betapars, mprior,sigmaprior
...
surfden = dblarr(2*pnts-1)
FOR i=0,2*pnts-4 DO $
   surfden(i) = int_tabulated(r_tot(i+1:2*pnts-2),2*nu_tot(i+1:2*pnts-2)*r_tot(i+1:2*pnts-2)/sqrt(r_tot(i+1:2*pnts-2)^2-r_tot(i)^2))
surfden(2*pnts-3:2*pnts-2)=0

;compute sigma outer
if (sigmaprior*sig_r_t(pnts-1) lt -sig_r_t(pnts-1)) then sigmaprior=-1.
sigma_tot=dblarr(2*pnts-1)
sigma_outer=sig_r_t(pnts-1)+(dindgen(pnts-1)+1)*sigmaprior*sig_r_t(pnts-1)/(rmax-rmin)*dr
sigma_tot(0:pnts-1)=sig_r_t
sigma_tot(pnts:2*pnts-2)=sigma_outer
