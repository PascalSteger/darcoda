...
module Nested
  ...
  !output files name
  character(LEN=255)physname,broot,rname,resumename,livename,evname,IS_Files(3)
  !output file units
...
!   end subroutine test
  ...
  subroutine nestRun(nest_IS, nest_mmodal, nest_ceff, nest_nlive, nest_tol, nest_ef, nest_ndims, nest_totPar, nest_nCdims, maxClst,  nest_updInt, nest_Ztol, nest_root, nchars, seed, nest_pWrap, nest_fb, nest_resume, nest_outfile, initMPI, nest_logZero, nest_maxIter,  loglike, dumper, context)

    implicit none

    integer nest_ndims,nest_nlive,nest_updInt,context,seed,i,nchars
    ...
    character(LEN=nchars) nest_root
    ...
    if( initMPI ) then
       ...
       ! if initMPI is set to false, although we compiled with MPI
       mpi_nthreads=1
       my_rank=0
    endif
    ...
    my_rank=0
#endif
    write(*,*)'after starting MPI'
    bogus = .false.
    write(*,*)'bogus=',bogus
    nest_nsc=50
    write(*,*)'nest_nsc=',nest_nsc

    write(*,*)'nest_nlive=',nest_nlive
    nlive=20
    write(*,*)'nlive=',nlive
    nlive=nest_nlive
    write(*,*)'nlive = ',nlive
    logZero=1.0d0*nest_logZero
    write(*,*)'bug here'
    Ztol=1.d0*nest_Ztol  !buggy, don't know why
    write(*,*)'some intermediate step'
    updInt=nest_updInt

    maxIter=nest_maxIter
    if(maxIter<=0) maxIter=huge(1)
    write(*,*)'first bunch of definitions done'
    ndims=nest_ndims
    totPar=nest_totPar
    nCdims=nest_nCdims
    debug=.false.
    ...
    write(*,*)'second bunch of definitions done'
  ...
  subroutine gen_initial_live(p,phyP,l,loglike,context)
    ...
    ! dumper not used in this function, removed from function call as well
    ! INTERFACE
    !    !the user dumper function
    !    subroutine dumper(nSamples, nlive, nPar, physLive, posterior, paramConstr, maxLogLike, logZ, INSlogZ, logZerr, context_pass)
    !      integer nSamples, nlive, nPar, context_pass
  ...
  subroutine clusteredNest(p,phyP,l,loglike,dumper,context)
    ...
    character(len=255) fName1, fName2 !file names
       ...
       if(ic_done(0)) then
          if(my_rank==0) then
             ...
             call pos_samp(Ztol,globff,broot,nlive,ndims,nCdims,totPar,multimodal,outfile,gZ,ginfo,ic_n,ic_Z(1:ic_n), &
                  ic_info(1:ic_n),ic_reme(1:ic_n),ic_vnow(1:ic_n),ic_npt(1:ic_n),ic_nBrnch(1:ic_n),ic_brnch(1:ic_n,:,1),&
                  phyP(:,1:nlive), l(1:nlive),evDataAll,IS,IS_Z,dumper,context)
