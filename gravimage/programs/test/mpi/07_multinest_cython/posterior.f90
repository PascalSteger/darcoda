...
contains
  ...
  subroutine pos_samp(Ztol,nIter,root,nLpt,ndim,nCdim,nPar,multimodal,outfile,globZ,globinfo,ic_n,ic_Z,ic_info,ic_reme, &
    ...
    if(outfile) then
       ...
       ic_nptloc(1:nClst)=int(temp(1:nClst,3))
       ...
       do
          ...
          !if(.not.mymerge(nClst,j,nPar,i,k,nCon(1:nClst),pts(:,1:k),stMu(1:nClst,1:j), &
  ...
  logical function mymerge(n,nCdim,nPar,npt,gnpt,nptx,pt,mean,sigma,locEv,nullEv,wt,nWt,llike)
    implicit none
    !input variables
