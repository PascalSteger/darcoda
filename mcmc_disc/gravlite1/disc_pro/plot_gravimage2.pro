...
IF (plotwhich EQ 'sim') THEN BEGIN
   ...
   basename = '/home/ast/read/dark/davev/justinmethod_output/Grav_image_disc/mwhr_r8500/met_long_ang'+patch+'12_mirr_log_bprior_constdm_rprior_kprior_mono'
...
IF (plotwhich EQ 'simple') THEN BEGIN
   basename = $
      '/home/ast/read/dark/davev/justinmethod_output/Grav_image_disc/simpletest/2comp_scale02029_kzmin0_7e3_8_mirr_bprior_constdm'
   nufile = basename + '_nu_dat_bin.txt'
   nufile2 = basename + '_nu_dat_bin2.txt'
   sigfile = basename + '_sigz_dat_bin.txt'
   sigfile2 = basename + '_sigz_dat_bin2.txt'
ENDIF
...
readcol,nufile,z_bin,nu_bin2,nu_bin_err2
readcol,sigfile,z_bin,sig_bin,sig_bin_err
readcol,sigfile2,z_bin,sig_bin2,sig_bin_err2
...
nupars2 = metarray(nt_nu+nt_kz+1:2*nt_nu+nt_kz,burnin:niter-1)
norm1 = metarray(nt_nu+nt_kz,burnin:niter-1)
norm2 = metarray(2*nt_nu+nt_kz+1,burnin:niter-1)
nn = niter-burnin
nn = nn(0)
...
nu_z2 = dblarr(zpnts,nn)
sig_z = dblarr(zpnts,nn)
sig_z2 = dblarr(zpnts,nn)
...
nu_zp2 = dblarr(np,zpnts)
sig_zp = dblarr(np,zpnts)
sig_zp2 = dblarr(np,zpnts)
...
FOR i=0L,nn-1 DO BEGIN
   ...
   nu_z2(*,pp) = nu(zpl,zp_kz,nupars2(*,i),quadratic,monotonic)
   sig_z(*,pp) = sigma_z(zpl,zp_kz,$
                         kzpars(*,i),blow,$
                         nupars(*,i),norm1(i),$
                         tpars(0:ntpars-1,i),tpars(ntpars:ntpars+2,i),$
                         quadratic,monotonic)
   sig_z2(*,pp) = sigma_z(zpl,zp_kz,$
                         kzpars(*,i),blow,$
                         nupars2(*,i),norm2(i),$
                         tpars(0:ntpars-1,i),tpars(ntpars:ntpars+2,i),$
                         quadratic,monotonic)
...
FOR i=0L,zpnts-1 DO BEGIN 
   ...
   indexnu2 = sort(nu_z2(i,0:nn-1))
   indexsg = sort(sig_z(i,0:nn-1))
   indexsg2 = sort(sig_z2(i,0:nn-1))
   ...
   FOR j=0L,np-1 DO BEGIN
      ...
      nu_zp2(j,i) = nu_z2(i,indexnu2(p(j)*(nn-1)))
      sig_zp(j,i) = sig_z(i,indexsg(p(j)*(nn-1)))
      sig_zp2(j,i) = sig_z2(i,indexsg2(p(j)*(nn-1)))
...
arraydump,basename+'_nu_zp.txt',[[zplt],[nu_zp(0,*)],[nu_zp(1,*)],[nu_zp(2,*)],$
                                 [nu_zp(3,*)],[nu_zp(4,*)]],6
arraydump,basename+'_nu_zp2.txt',[[zplt],[nu_zp2(0,*)],[nu_zp2(1,*)],[nu_zp2(2,*)],$
                                 [nu_zp2(3,*)],[nu_zp2(4,*)]],6
arraydump,basename+'_sig_zp.txt',[[zplt],[sig_zp(0,*)],[sig_zp(1,*)],[sig_zp(2,*)],$
                                  [sig_zp(3,*)],[sig_zp(4,*)]],6
arraydump,basename+'_sig_zp2.txt',[[zplt],[sig_zp2(0,*)],[sig_zp2(1,*)],[sig_zp2(2,*)],$
                                  [sig_zp2(3,*)],[sig_zp2(4,*)]],6
...
oname = basename + '_sigmaz2' + filetype
IF (devtype EQ 'PS') THEN $
   device,filename=oname,/color,$
          xsize=16,ysize=16,/encapsul
...
FOR i=0L,np-1 DO BEGIN
   IF (i EQ 0) THEN BEGIN
      plot,zpl,sig_zp2(i,*),title='!6',xtitle='z(kpc)',$
           ytitle='!7r!6!dz!n [km/s]',$
           yrange=[ysigmin,ysigmax],xrange=[0,max(z_dat)],$
           linestyle = pstyle(i)

      ;Overplot data:
      oploterror,z_bin,sig_bin2,sig_bin_err2,color=2,errcol=2
   ENDIF ELSE $
      oplot,zpl,sig_zp2(i,*),linestyle=pstyle(i)
ENDFOR
...
FOR i=0L,np-1 DO BEGIN
   ...
   pnorm = 1.
   IF (i EQ 0) THEN BEGIN
      plot,zpl,nu_zp2(i,*)/pnorm,title='!6',xtitle='z(kpc)',/ylog,$
           ytitle='!7m!6 [units]',yrange=[ynumin,ynumax],$
           xrange=[0,max(z_dat)],$
           linestyle = pstyle(i)
      
      ;Overplot data:
      oploterror,z_bin,nu_bin2,nu_bin_err2,color=2,errcol=2
   ENDIF ELSE $
      oplot,zpl,nu_zp2(i,*)/pnorm,linestyle=pstyle(i)
ENDFOR
