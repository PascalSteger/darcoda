     ...
     do ind=1,twotondim
        ...
        do i=1,ngrid
           ...
           if(ok(i))then
              ...
              if(eta_sn>0.0d0)ndebris_tot=ndebris_tot+4*ndebris*nstar(i)
           endif
        enddo
           ...
           if(zg<metal_thresh)then
             ...
             do while(mass_gas>0.)
               ...
               if(mass_tmp<mass_cutoff)then
                 call clean_evolved_masses(mass_tmp,mass_ll,mdebris,mass_pi,mass_bh,mass_rad)
                 mass_gas=0.
               else
                 call find_evolved_masses(mass_tmp,mass_ll,mdebris,mass_pi,mass_bh, &
                      mass_rad,zdebris,zdebris2)
           ...
           if(eta_sn>0.0)then
              ...
              do ideb=1,1!ndebris_tot,nvector
                 ...
                 do j=1,ndeb
                    ...
                    ! radiation debris
                    tp(ind_debris3(j))=birth_epoch  ! Birth epoch
                    mp(ind_debris3(j))=mass_rad      ! Mass
                    levelp(ind_debris3(j))=ilevel   ! Level
                    idp(ind_debris3(j))=-index_star ! black hole Identity
                    xp(ind_debris3(j),1)=x
                    xp(ind_debris3(j),2)=y
                    ...
                    vp(ind_debris3(j),2)=v 
                    vp(ind_debris3(j),3)=w 
                    if(metal)zp(ind_debris3(j))=-1. !+ zdebris ! Commented out by ACB. Added again below                
...
subroutine radiation(ind_grid,ind_part,ind_grid_part,ng,np,ilevel,vdebris)
  ...
  integer ,dimension(1:nvector),save::igrid,icell,indp,kg
  real(dp),dimension(1:3)::skip_loc
  real(dp)::scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v,SolarMassCode,thisMass
  real(dp)::rstromgren,QH,Xfrac=0.76
  real(dp)::twopi,mH=1.67e-24
  ...
  ! Update hydro variables due to feedback 
  do j=1,np
     thisMass=meff(J)*vol_loc/SolarMassCode
     QH=exp(111.+log(thisMass)+log(1.-exp(-thisMass/100.))+log(1.-exp(-thisMass/20.))+log(1.-exp(-thisMass/15.)))
     rstromgren=(QH/3e-13*3./(2*twopi)/(uold(indp(j),1)*Xfrac/mH*scale_d)**2)**(1./3.)
     ethermal=QH*2.2e-11/(2*twopi*rstromgren**2*3e10*scale_v**2*scale_d) !# 2.2e-11 approx hnu in erg
     print *, QH,QH*2.2e-11,rstromgren,ethermal,&
      meff(j)*(3.8e33*scale_t/2e33/scale_v**2)*(meff(J)*vol_loc/SolarMassCode)**4*dtnew(ilevel)
     !ethermal=meff(j)*(3.8e33*scale_t/2e33/scale_v**2)*(meff(J)*vol_loc/SolarMassCode)**4*dtnew(ilevel)
