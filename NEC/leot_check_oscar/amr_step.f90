recursive subroutine amr_step(ilevel,icount)
  ...
  if(ilevel==levelmin)then
    ...
    ! Dump lightcone
     ...
     if(output_mode.gt.0) then
        if(mod(nstep_coarse,output_mode)==0)then
           if(.not.ok_defrag)then
              call defrag
           endif
           call backup_all
  ...
  !Movie dump without synched levels
  if(movie) then
     ! Conversion factor from user units to cgs units
     call units(scale_l,scale_t,scale_d,scale_v,scale_nH,scale_T2)

!     tcurr=t*scale_t/365./24./60./60/1.0d6  !Time in Myr
     tcurr=t

     if(tcurr>=movout(imov))then
        call output_frame()
     endif
  end if
  ...
  if(poisson)then
     ...
     ! Oscar: Reset Prad at ilevel here
!     if(radpressure) then
!        call reset_prad(ilevel)
!     endif
