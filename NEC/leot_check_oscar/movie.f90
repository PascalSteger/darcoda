...
subroutine output_frame()
  ...
  real(dp)::dx_frame,dy_frame,dz_frame,dx,dx_loc
  ...
  real(kind=8),dimension(:,:,:),allocatable::data_frame,data_frame_all,data_frame_perp
  real(kind=4),dimension(:,:),allocatable::data_single,data_single_perp
  ...
  real(kind=8) :: maxtemp_local, maxtemp_global
  real(kind=8) :: maxpressure_local, maxpressure_global
  ...
  maxtemp_local = 0.0
  maxtemp_global = 0.0
  maxpressure_local = 0.0
  maxpressure_global = 0.0
...
!  if(myid==1)call system(filecmd)
  ...
  moviefile4 = trim(moviedir)//'cr_'//trim(istep_str)//'.map'
  ...
  ! Allocate image
  allocate(data_frame(1:nx_frame,1:ny_frame,1:5))
  data_frame=0d0
              ...
              if(ok(i))then
                 ...
                 do ii=imin,imax
                    ...
                    do jj=jmin,jmax
                       ...
                       if (temp .gt. maxpressure_local) maxpressure_local = temp

                       temp=temp/uold(ind_cell(i),1)*scale_T2 !temperature in K

                       if (temp .gt. maxtemp_local) maxtemp_local = temp
  ...
  !Reduce max temp and pressure:
#ifndef WITHOUTMPI
  call MPI_ALLREDUCE(maxtemp_local,maxtemp_global,1,MPI_DOUBLE_PRECISION,MPI_MAX,MPI_COMM_WORLD,info)
  call MPI_ALLREDUCE(maxpressure_local,maxpressure_global,1,MPI_DOUBLE_PRECISION,MPI_MAX,MPI_COMM_WORLD,info)
#else
  maxtemp_global = maxtemp_local
  maxpressure_global = maxpressure_local
#endif

  if (myid .eq. 1) then
     write(*,*)"   At t=",t,", max temperature=: ",maxtemp_global
  endif
  ...
  !Just swap the y and z thicknesses:
  delx=deltax_frame(1)+deltax_frame(2)/aexp
  dely=deltay_frame(1)+deltay_frame(2)/aexp
  delz=deltaz_frame(1)+deltaz_frame(2)/aexp
  xleft_frame=xcen-delx/2.
  xright_frame=xcen+delx/2.
  yleft_frame=ycen-dely/2.
  yright_frame=ycen+dely/2.
  ...
  ! Allocate image
  allocate(data_frame_perp(1:nx_frame,1:ny_frame,1:5))
  data_frame_perp=0d0

  dx_frame=delx/dble(nx_frame)
  dz_frame=delz/dble(ny_frame)
        ...
        do ind=1,twotondim
           ...
           do i=1,ngrid
              if(ok(i))then
                 ...
                 !n.b. this now corresponds to the z direction (Craig)
                 if(zleft>zleft_frame)then
                    jmin=min(int((zleft-zleft_frame)/dz_frame)+1,ny_frame)
                 else
                    jmin=1
                 endif
                 jmax=min(int((zright-zleft_frame)/dz_frame)+1,ny_frame)
                 
                 ! Fill up map with projected mass
#if NDIM>2                 
                 !n.b. this now corresponds to the y direction (Craig)
                 dy_cell=min(yright_frame,yright)-max(yleft_frame,yleft)
#endif
                 do ii=imin,imax
                    xxleft=xleft_frame+dble(ii-1)*dx_frame
                    xxright=xxleft+dx_frame
                    dx_cell=min(xxright,xright)-max(xxleft,xleft)
                    do jj=jmin,jmax
                       zzleft=zleft_frame+dble(jj-1)*dz_frame
                       zzright=zzleft+dz_frame
                       dz_cell=min(zzright,zright)-max(zzleft,zleft)
                       ! Intersection volume
                       dvol=dx_cell*dz_cell
#if NDIM>2                 
                       dvol=dvol*dy_cell
#endif
                       data_frame_perp(ii,jj,1)=data_frame_perp(ii,jj,1)+dvol*uold(ind_cell(i),1)
                       data_frame_perp(ii,jj,2)=data_frame_perp(ii,jj,2)+dvol*uold(ind_cell(i),1)**2
                       ...
                       data_frame_perp(ii,jj,3)=data_frame_perp(ii,jj,3)+dvol*uold(ind_cell(i),1)*temp !mass weighted temperature

                       if(metal)then
                          data_frame_perp(ii,jj,4)=data_frame_perp(ii,jj,4)+dvol*uold(ind_cell(i),6)
                       endif
...
#ifndef WITHOUTMPI
  ! Reduce the edge on frame:
  allocate(data_frame_all(1:nx_frame,1:ny_frame,1:5))
  call MPI_ALLREDUCE(data_frame,data_frame_all,nx_frame*ny_frame*5,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)
  data_frame=data_frame_all
  deallocate(data_frame_all)

  allocate(data_frame_all(1:nx_frame,1:ny_frame,1:5))
  call MPI_ALLREDUCE(data_frame_perp,data_frame_all,nx_frame*ny_frame*5,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)
  data_frame_perp=data_frame_all
  deallocate(data_frame_all)
#endif
  ...
  do ii=1,nx_frame
     do jj=1,ny_frame
        data_frame(ii,jj,2)=scale_nH*data_frame(ii,jj,2)/data_frame(ii,jj,1)  !in mH/cc
        data_frame(ii,jj,3)=data_frame(ii,jj,3)/data_frame(ii,jj,1) !Already in Kelvin
        if(metal)then
           data_frame(ii,jj,4)=data_frame(ii,jj,4)/data_frame(ii,jj,1) !In met
        endif


        data_frame_perp(ii,jj,2)=scale_nH*data_frame_perp(ii,jj,2)/data_frame_perp(ii,jj,1)  !in mH/cc
        data_frame_perp(ii,jj,3)=data_frame_perp(ii,jj,3)/data_frame_perp(ii,jj,1) !Already in Kelvin
        if(metal)then
           data_frame_perp(ii,jj,4)=data_frame_perp(ii,jj,4)/data_frame_perp(ii,jj,1) !In met
        endif
  ...
  if(myid==1)then
     ...
     if(metal)then
        ...
        data_single_perp=data_frame_perp(:,:,4)
        rewind(ilun)  
        write(ilun)aexp,delx,dely,delz
        write(ilun)nx_frame,ny_frame
        write(ilun)data_single
