subroutine init_amr
 ...
 bisection_or_ordering:if(ordering /= 'bisection') then ! use usual ordering machinery
  ...
  if(nrestart>0)then
     ...
     if(myid==1)write(*,*) 'iout=',ifout,'ifout=',ifout
...
!     endif
  ...
  if(imovout>0) then
     allocate(movout(1:imovout))
     do i=1,imovout
        movout(i)=(dble(i)/dble(imovout))*tfinal !Over 100 Myr
     enddo
  endif

!     if(imovout>0) then  !Current frame number
!       do i=2,imovout
!           if(aexp>movout(i-1).and.aexp<movout(i)) then
!              imov=i !or i-1?
!           endif
!        enddo
!     endif
     if(myid==1)write(*,*) "Frame number, aexp ",imov, movout(imov)
