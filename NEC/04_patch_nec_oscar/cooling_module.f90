...
module cooling_module
  ...
  real(dp),parameter::kBeV=8.61343e-5,ndenfloor=0.0
  real(dp),save::ySingle(neq_spec),weight_spec(neq_spec)
  real(dp),save::k1,k2,k3,k4,k5,k6,k7,k8,k9,k10,k11,k12,k13,k14,k15,k16
  real(dp),save::k17,k18,k19,k20,k21,k22,k23,k24,k25,k26,k27,k28,k29=0.0,k30,k31
  real(dp),save::k7_prime,k11_prime,k14_prime,k15_prime,k16_prime,k18_prime,k30_prime,k31_prime
  real(dp),save::temperatureCMB
  real(dp),save::R22,R23,R24,R25,R26,R27,R28
#endif
...
! so the code compiles for nonhydro jobs.
  real(dp),save::ism_N=1d21,ism_grain_a=1e-4
  real(dp),save::step_frac=0.1,fixedMu=1.22
...
#ifdef NONEQCHEM
  real(dp)::scale_l,TTT,dummy,timeCooling,muChem,nTot,TR
  integer::Nsub,i
  logical::doYSINGLE
#endif
...
!#######################################################################
   ...
   subroutine nonEquiChemUseSingle(y)
    implicit none
    integer i
    real(dp)::nHSingle,nTot
    real(dp)::y(neq_spec-1)
    nTot=0.0
    do i=1,neq_spec-1
...
!#######################################################################
   real(dp) function nonEquiChemMu(y)
    implicit none
    real(dp)::nH,nHe,nH2,nE,nTot
    real(dp),dimension(neq_spec)::y
...
!#######################################################################
   real(dp) function  nonEquiChemStep(f,y)
    implicit none
      integer :: i
      real(dp),dimension(neq_spec)::y,f
      real(dp)::h
      h=0.0
      do i=1,neq_spec
...
!#######################################################################
   subroutine solveNonEqui(y,dt,Nsub)
    ...
    implicit none
    integer::i,j,Nsub,N
    real(dp),dimension(neq_spec)::yold,yc,dydt,y
    real(dp),dimension(neq_spec,neq_spec)::cc,dfdy
    real(dp)::time,oldTime,c,h,dt,hold,maxvar,coolColl
    logical::OK
...
!#######################################################################
   subroutine solveNonEqui_1it(y,h,maxvar)
    ...
    implicit none
    integer::i,j,Nsub,N
    real(dp),dimension(neq_spec)::yold,dydt,y
    real(dp),dimension(neq_spec,neq_spec)::cc,dfdy
    real(dp)::h,maxvar
    logical::OK
...
!#######################################################################
   subroutine solveNonEqCooling(nh,yc,t2,zsolar,dt,deltat2,ncell)
     ...
     implicit none
       integer::ncell
       real(dp)::dt,timeCool,ncr,lrot,lvib
       real(dp),dimension(1:ncell)::nh,t2,deltat2,zsolar,wcool
       real(dp),dimension(1:nvector,1:neq_spec)::yc,yc_old
       real(dp)::dydt(1:neq_spec)
       real(dp)::h2c,h2c_prime,wcool_old,h2lte_prime
       real(dp)::tempk1,tempk0,lvib0,lvib1,lrot1,lrot0,h2c1,h2c0

       real(dp)::fact,dlog_nh,dlog_t2,t2eq,precoeff,h,h2,h3
       real(dp)::metal,cool,heat,w1t,w2t,w11,w12,w21,w22,yy,yy2,yy3
       real(dp)::metal_prime,cool_prime,heat_prime,nHtot
       real(dp)::lambda,lambda_prime,lambda_coll,lambda_coll_prime
       real(dp)::fa,fb,fprimea,fprimeb,alpha,beta,gamma
       real(dp),dimension(1:ncell)::rgt,lft,tau,tau_old,nnh
       real(dp),dimension(1:ncell)::time,time_old,fach,zzz,tau_ini
       real(dp),dimension(1:ncell)::w1h,w2h,wmax,time_max
       real(dp)::varmax=4.0,maxvar,dtcool,dtcool_old,muc,h2lte
       real(dp)::a(5),b(5),tempK
...
!#######################################################################
   real(dp) function neq_H2CollCool(y)
    implicit none
    real(dp)::y(neq_spec)
    logical::noTime
...
!#######################################################################
   real(dp) function neq_H2CollCool_prime(y)
    implicit none
    real(dp)::y(neq_spec)
...
!#######################################################################
   real(dp) function neq_H2RadCool(y,a)
    implicit none
    real(dp)::y(neq_spec),a(5)
...
!#######################################################################
   real(dp) function neq_H2RadCool_prime(y,b)
    implicit none
    real(dp)::y(neq_spec),b(5)
...
!#######################################################################
   subroutine nonEquiChemRates(Tin)
    implicit none
    integer::i,j,n
    real(dp):: sqrtT,sqrtT5,T6,T5,T3,TeV,T,log10T,Tin
...
!#######################################################################
   subroutine nonEquiChemOdes(dydt,y)
    implicit none
    integer::n
    real(dp)::dydt(neq_spec),y(neq_spec)
...
!#######################################################################
   subroutine nonEquiChemJac(dfdy,y)
    implicit none
    real(dp)::dfdy(neq_spec,neq_spec),y(neq_spec)
...
! H2 cooling
!#######################################################################
   real(dp) function HHonH1cool(t)
    implicit none
    real(dp)::t,logt
    real(dp), parameter::a0=-16.818342,a1=37.383713,a2=58.145166,   &
...
!    if (t==100.0)then
   ...
   real(dp) function HHonH1coolPrime(t)
    implicit none
    real(dp)::t,logt
    real(dp), parameter::a0=-16.818342,a1=37.383713,a2=58.145166,   &
     a3=48.656103,a4=20.159831,a5=3.8479610,                      &
...
!#######################################################################
   real(dp) function HHonHHcool(t)
    implicit none
    real(dp)::t,logt
    real(dp), parameter::a0=-23.962112,a1=2.09433740,a2=-0.77151436,&
...
!    if (t<1d4)then
   ...
   real(dp) function HHonHHcoolPrime(t)
     implicit none
     real(dp)::t,logt
    real(dp), parameter::a0=-23.962112,a1=2.09433740,a2=-0.77151436,&
     a3=0.43693353,a4=-0.14913216,a5=-0.033638326
...
!#######################################################################
   real(dp) function HHonHe1cool(t)
    implicit none
    real(dp)::t,logt
    real(dp), parameter::a0=-23.689237,a1=2.1892372,a2=-0.81520438, &
...
!    if (t>100.0.and.t<10000.0)then
   ...
   real(dp) function HHonHe1coolPrime(t)
    implicit none
    real(dp)::t,logt
    real(dp), parameter::a0=-23.689237,a1=2.1892372,a2=-0.81520438, &
     a3=0.29036281, a4=-0.16596184,a5=0.19191375
...
!#######################################################################
   real(dp) function HHonH2cool(t)
    implicit none
    real(dp)::t,logt
    real(dp), parameter::a0=-21.716699,a1=1.3865783,a2=-0.37915285,&
...
!    if (t>100.0.and.t<1d4)then
   ...
   real(dp) function HHonH2coolPrime(t)
    implicit none
    real(dp)::t,logt
    real(dp), parameter::a0=-21.716699,a1=1.3865783,a2=-0.37915285,&
     a3=0.11453688,a4=-0.23214154,a5=0.058538864
...
!#######################################################################
   real(dp) function HHonEcool(t)
    implicit none
    real(dp)::t,logt
    real(dp), parameter::a0=-34.286155,a1=-48.537163,a2=-77.121176,&
...
!    if (t>100.and.t<=2d2)then
   ...
   real(dp) function HHonEcoolPrime(t)
    implicit none
    real(dp)::t,logt
    real(dp), parameter::a0=-34.286155,a1=-48.537163,a2=-77.121176,&
     a3=-51.352459,a4=-15.169160,a5=-0.98120322,                 &
...
!#######################################################################
   real(dp) function h2coolDiff(t,iSpec)
     implicit none
     real(dp)::t
     integer::iSpec
     if(iSpec==inH1)then
...
!#######################################################################
   real(dp) function h2cool_prime(t)
      real(dp)::t,n,logt,h
...
!#######################################################################
   real(dp) function crossSection(hnu,reaction)
...
! hnu=11.26eV
    implicit none
    integer::reaction
    real(dp),parameter::orthoPara=3.0
    real(dp)::sigW0,sigW1,sigL0,sigL1,hnu,hnuTh,logNu,x
...
!#######################################################################
   real(dp) function taux_diss(ispec,J0)
    implicit none
    integer::ispec
    real(dp)::J0,e_i,e,de,error,integ
    if(ispec==25  )then
     e_i = 0.755*eV
...
!#######################################################################
   real(dp) function J0BlackBody(e,T)
    implicit none
    real(dp) ::T,e
...
!#######################################################################
   real(dp) function hxh_diss(T)
! this routine assumes a Planckian radiation field. From T Abel et al. (1997).
! referenced Sauval & Tatum (1984)
    implicit none
    real(dp), parameter::c0=9.9835,c1=-6.64e-2,c2=-1.4979,c3=-1.95e-2,c4=0.7486,d=2.6508
    real(dp):: T, x, KLTE,logX
    x=5040.0/T
    logX=log10(x)
...
!#######################################################################
   real(dp) function taux_cmb(ispec,eThresh,a)
    implicit none
    integer::ispec
    real(dp)::eThresh,e,de,error,integ,a
    integ = 0.0
    e = eThresh
    de = e/100.0
    error = 1.0
...
!#######################################################################
   ...
   ! CMB and then J0simple-like dissociation and ionization rates
    implicit none
    real(dp)::a
    real(dp)::J0,TR !instead of real(dp)
    TR=temperatureCMB
    if (a<1.0/7.0)then
...
!#######################################################################
   ...
   ! Invert an NXN matrix
      ...
      real(dp)::a(np,np),big,dum,pivinv
      logical::OK
      OK=.true.
      do j=1,n ! 11
       ipiv(j)=0
...
!#######################################################################
   subroutine isobaricTest()
    ...
    real(dp)::y(neq_spec),Tinit=500,p,T,pInit,time,lambda,Told,nInit=0.03
    real(dp)::wcool,dt,mu,h2c,h2c_prime,lambda_prime,time_old,nTot,dummy,lambda_op
    real(dp)::nHydrogen,h2c_diff,varmax=4.0,timeCool,muOld,thisrho,rhodot,tff
    real(dp)::ncr,ncrH,ncrHH,ncrHe,nHatom,nHmolecule,nHelium,lrot,lvib,thisrho0,thisoldrho
    real(dp)::a(5),b(5),tempK,lambda_coll,lambda_coll_prime,maxvar,nHtot
