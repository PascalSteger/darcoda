...
subroutine star_formation(ilevel)
  ...
  mass_cutoff=10d0*2d33/scale_d/scale_l**3
     ...
     do ind=1,twotondim
        ...
        do i=1,nnew
           ...
           if(zg<metal_thresh)then
             ...
             do while(mass_gas>0.)
               fboom=get_random()
               fboom=fboom**(1./2.3) ! use an IMF like distribution
               mass_tmp=mass_gas*(1.-fboom)+mass_cutoff
           ...
           if(eta_sn>0.0)then
              ...
              do ideb=1,ndebris_tot,nvector
                 ...
                 do j=1,ndeb
                    ...
                    if(metal)zp(ind_debris1(j))=zg + yield ! Commented out by ACB. Added again below
                    ...
                    if(metal)zp(ind_debris2(j))=-(zg + yield) !negative yield gives PI SNe
...
subroutine feedback(ilevel)
 ...
 !             if(idp(ipart).eq.0.and.tp(ipart).lt.(t-t0))then
              if((idp(ipart).eq.0.and.tp(ipart).lt.(t-t0)).or. &
                 (idp(ipart).lt.0.and.mp(ipart).eq.0.    ))then
                 npart2=npart2+1
              endif
        ...
        if(npart2>0)then        
           ...
           do jpart=1,npart1
              ...
              !if(idp(ipart).eq.0.and.tp(ipart).lt.(t-t0))then
              if((idp(ipart).eq.0.and.tp(ipart).lt.(t-t0)).or. &
                 (idp(ipart).le.0.and.mp(ipart).eq.0.    ))then
                 ip=ip+1
                 ind_part(ip)=ipart
                 ind_grid_part(ip)=ig   
              endif
...
subroutine sn2(ind_grid,ind_part,ind_grid_part,ng,np,ilevel,vdebris)
  ...
  if(metal)then
     do j=1,np
        uold(indp(j),6)=uold(indp(j),6)+meff(j)*abs(zp(ind_part(j))) ! always same debris
     end do
  endif
...
!################################################################
!################################################################
subroutine find_evolved_masses(mm,ml,md,mp,mb)
 use amr_commons
 implicit none

  real*8::m1,m2,m3,m4,m5,mm,ml,md,mb,mp,scale_mass
...
subroutine clean_evolved_masses(mm,ml,md,mp,mb)
 ...
 implicit none
  ...
  real*8::m1,m2,m3,m4,m5,mm,ml,md,mb,mp
