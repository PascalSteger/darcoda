program amr2map
  ...
  real(KIND=8)::xxmin,xxmax,yymin,yymax,zzmin,zzmax,dx,dy
...
! ROM: comment the single follwing line for old stuff
  ...
  write(*,*)'Norm=',sum(grid(lmax)%map(imin:imax,jmin:jmax))/(imax-imin+1)/(jmax-jmin+1)
