  ...
  if(ok_file)then
     ...
     do ivar=1,nvar
        ...
        if(ok_file3)then
           ...
           if(myid==1)write(*,*)'Reading file f '//TRIM(filename)
           if(multiple)then
              ilun=ncpu+myid+10
...
#ifdef NONEQCHEM
           if(ivar>ndim+3)then
              ! Get initial chemical abundances
              print *,"get initial chemical abundances"
              call nonEquiChemUseSingle(yd)
              init_array=yd(ivar-(ndim+3))
           end if
#endif
        endif
