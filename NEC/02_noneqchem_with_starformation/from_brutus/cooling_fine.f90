subroutine cooling_fine(ilevel)
  ...
  if(cooling.and.ilevel==levelmin.and.cosmo)then
     if(myid==1)write(*,*)'Computing new cooling table (NEC)'
     call set_table(dble(aexp))
  end if
#ifdef NONEQCHEM
  call output_abundances()
#endif
...
subroutine coolfine1(ind_grid,ngrid,ilevel)
...
#ifdef NONEQCHEM
  real(8)::timeTemp,mmw
  real(8)::yd(1:neq_spec),tempT2(1:nvector)
  real(8)::scale_nH2,tempTemp,sqrtDist,mu,denSum
  logical::justSingle
  real(8),dimension(1:nvector,1:neq_spec)::yarray
  real(8),dimension(1:neq_spec)::sumy
  integer::Nsub,NN,ii
#endif
  ...
  real(8)::scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v
  ...
  do ind=1,twotondim
     ...
     ![PS]  output for recombination test: overall abundances of all elements 
     !      summed over all cells
     !TODO: weight by volume of cell, get outside loop!
#endif
...
end subroutine output_abundances()
#endif
