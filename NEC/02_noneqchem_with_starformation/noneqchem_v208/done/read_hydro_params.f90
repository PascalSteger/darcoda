subroutine read_hydro_params(nml_ok)
  use amr_commons
  use hydro_commons
  use cooling_module, only: step_frac,chemistry,print_recomb &
    ,isobaric_test,fixedMu,dust,ism_N,ism_grain_a
  implicit none
#ifndef WITHOUTMPI
  include 'mpif.h'
#endif
  ...
  namelist/physics_params/cooling,haardt_madau,metal,isothermal,bondi &
       ...
       & ,orig_star_top,orig_star_mass,n_starIII,n_starII
  namelist/noneqchem_params/step_frac,fixedMu &
       & ,ism_N,ism_grain_a,chemistry &
       & ,dust,print_recomb,isobaric_test
...
105 continue
  rewind(1)
  read(1,NML=noneqchem_params,END=106)
106 continue

  !--------------------------------------------------
  ! Check for correct chemistry set up
  !--------------------------------------------------
#ifndef NONEQCHEM 
  if(chemistry)then 
   nml_ok=.false. 
   if(myid==1)print *,'You must compile with NONEQCHEM to use the chemistry routines.' 
  endif 
#else 
  if(.not.chemistry)then 
   nml_ok=.false. 
   if(myid==1)print *,'You compiled with the NONEQCHEM routines.  Did you mean to do so?' 
   if(myid==1)print *,'chemistry in the noneqchem_params namelist is currently set to false.' 
  endif 
#endif 
