...
subroutine star_formation(ilevel)
  ...
  integer ::ntot,ntot_all,info,nstar_corrected,ideb,ndeb,ii,nn,noboom
#ifdef SOLVERhydro
  integer ::imetal=6
#endif
#ifdef SOLVERmhd
  ...
  real(dp)::d,x,y,z,u,v,w,e,zg,vdisp,dgas,should_i_blow,fboom
  real(dp)::mstar,dstar,tstar,nISM,nCOM,d0III,d0II
...
!       & /(scale_nH*aexp**3)*vol_min
  mstar=orig_star_mass*2d33/(scale_d*scale_l**3)
  star_mass=mstar
  dstar=mstar/vol_loc
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        do i=1,ngrid
           d=uold(ind_cell(i),1)
           d0=d0III
           if(uold(ind_cell(i),6)/uold(ind_cell(i),1)>metal_thresh)d0=d0II
        ...
        do i=1,ngrid
           ...
           if(ok(i))then
              ...
              d0=d0III
              t0=t0III
              if(uold(ind_cell(i),6)/uold(ind_cell(i),1)>metal_thresh)then
                d0=d0II
                t0=t0II
              endif
              tstar=t0*sqrt(d0/d)
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        do i=1,nnew
             ...
             fboom=should_i_blow(nstar_tot-nnew+i)
             ...
             print *, mp(ind_part(i)),idp(ind_part(i)),noboom,fboom
...
subroutine sn2(ind_grid,ind_part,ind_grid_part,ng,np,ilevel,vdebris)
  ...
  do j=1,np
     if(ok(j))then
        ...
        !if(zp(ind_part(j))>metal_thresh)then
        if(idp(ind_part(j))>0)then
          meff(j)=star_mass*0.1/vol_loc
        else
        ...
        !if(zp(ind_part(j))>metal_thresh)then
        if(idp(ind_part(j))>0)then
          meff(j)=star_mass*0.1/vol_loc/dble(twotondim)
        else
...
!################################################################
!################################################################
real*8 function should_i_blow(idum)
!* math can be done in integer if two comments Cs are moved
!* see numerical recipes
      Implicit None
      Integer iff,i,inext,inextp,k,ii,idum
      ...
      MA(INEXT)=MJ
      should_i_blow=(MJ*FAC*2.)

      RETURN
end function should_i_blow
