...
subroutine star_formation(ilevel)
  ...
  logical ,dimension(1:nvector),save::ok,ok_new=.true.,ok_true=.true.,zswitch,flagreal
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        ! Update linked list for debris ![PS] included from pm/star_formation: check for other handling of debris in Aaron Boley's manuscript
        if(f_w>0)then
