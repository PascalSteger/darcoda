...
! so the code compiles for nonhydro jobs.
  ...
  logical,save::chemistry=.false.,print_recomb=.false.,isobaric_test=.false.
...
!=======================================================================
  ...
  do while (aexp < aend)
     ...
     print *,"debug: after set_rates"
#ifdef NONEQCHEM
    if(doYSINGLE)then
      print *,"debug: inside doYSINGLE"
      scale_l = aexp * boxlen_ini * 3.08d24 / (h0/100)
      do i=1,neq_spec
      ...
      if(print_recomb)then
         print *,"debug: inside print_recomb"
         if (myid==1)then
...
!=======================================================================
  ...
  integer :: niter
  logical :: if_write_result=.false. ![PS] for recombination test? use false as default
...
![PS] possible place to read out table%nspec for recombination test
   ...
   subroutine solveNonEqui(y,dt,Nsub)
    ...
    logical::OK
      ...
      do 
         ...
         ![PS] all the following write statements
      ...
      enddo
      y=yc
      ![PS] the following two write statements, for rectest? no use, CHEMINFO just after
      write(*,*)Nsub
