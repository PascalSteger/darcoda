module tree_data
 implicit none
 public
 ! bucketmax must be bigger than number of particles
 integer,parameter :: bucketmax=100000,maxneigh=64
...
recursive subroutine insert_tree_item(part,parent)
    ...
    type(item)::part
    type(node), pointer :: parent
    real*8::down,left,front,dx,x,y,z,dxh
    x=part%x
    y=part%y
    z=part%z
    down=parent%down
...
!              ichild,child%index,child%level,n
     ...
     if(n>0)then
       ...
       current=>member
       do while(associated(current))
         print *, part(current%index)%x,    &
                  part(current%index)%y,    &
                  part(current%index)%z,    &
                  part(current%index)%mass, &
                  part(current%index)%rho,  &
                  part(current%index)%h,    &
                  part(current%index)%level,&
...
recursive subroutine get_neighbors(maxlevel,maxneigh,npart,part,parent)
   implicit none
   integer npart,maxneigh,maxlevel,ichild,thischild,nlocal
   type(item),dimension(:):: part
   type(node), pointer::parent,child
   type(llist), pointer::member
   ...
   if(parent%level==maxlevel)then
       ...
       call gather(thischild,maxneigh,nlocal,part,member,parent)
...
!       print *, ichild,child%level,maxlevel,parent%level,parent%index
!       print *," located ",parent%xc,parent%yc,parent%zc,parent%dx
!       call gather(thischild,maxneigh,part,member,parent)
!     else
...
!     endif
      ...
      call gather(ichild,maxneigh,nlocal,part,member,parent)
    endif
   enddo
   endif
end subroutine

recursive subroutine gather(ichild,maxneigh,nlocal,part,member,parent)
   implicit none
   integer maxneigh,ineigh,i_out,i_in,ichild,dist2maxId,i,nlocal
   ...
   type(llist), pointer::outer,member
   type(node), pointer::parent
   logical findEdge
...
subroutine top_gather(maxneigh,ipart,npart,part)
    ...
    integer nneigh,maxneigh,dist2maxId,npart
    real*8 dist2max,xo,yo,zo,xi,yi,zi,h,dist2,kernel
...
program main
 ...
 use tree_data
 implicit none
 integer :: i,id,level,maxlevel,ilevel,&
            ierror,iheader,nentry
 real*8 :: newtime,oldtime,left,down,front
...
! print *,"#Built main tree after ",newtime-oldtime
 oldtime=newtime
...
! print *, "#id tree found maxlevel ",maxlevel,newtime-oldtime
 ...
 enddo
 call cpu_time(newtime)
! print *, "# Through tree except trunk after",newtime-oldtime
 oldtime=newtime
 do i=1,nentry
