...
nnzj1 = []; nnzj2 = [];
for i=1:nbins    
    bnzj1 = mvir2'>10*min(ahf_mvir(ahf_npart1==min(ahf_npart1))) & mvir2'<mres & ibin==i & ~isnan(acp);
    nnzj1(i) = sum(bnzj1);
...
hold on;
scatter(r2(mvir2'>10*min(ahf_mvir(ahf_npart1==min(ahf_npart1))) & mvir2'<mres),acp(mvir2'>10*min(ahf_mvir(ahf_npart1==min(ahf_npart1))) & mvir2'<mres));
axis([0,rmax,0,1]);
