...
;Fit sigma out to half the maximal data radius
...
if (populations eq 2) then pars = [nupars1,nupars2,Mpars,betapars1,betapars2] else pars = [nupars1,Mpars,betapars1]
...
if (populations eq 2) then parstep = [nuparstep1,nuparstep2,Mparstep,betaparstep1,betaparstep2] else parstep = [nuparstep1,Mparstep,betaparstep1]
...
IF (logprior EQ 'yes') THEN BEGIN
   ...
   lpars(populations*nt_nu+nt_M:populations*nt_nu+nt_M+populations*nt_beta-1) = pars(populations*nt_nu+nt_M:populations*nt_nu+nt_M+populations*nt_beta-1) ;linear sampling for beta
   lparstep = lpars 
...
accrejtollow = 0.2              ;Acceptance/rejection rate
accrejtolhigh = 0.30            ;
chisqtol = 10.                  ;Parameters to end initphase 
endcount = 100                  ;
...
;   norm2 = parst(2*nt_nu+nt_M+2*nt_beta+1)
   ...
   sig1_r = sigma_r(r_sigdat,rp_nu,rp_M,rp_beta,Mpars,nupars1,betapars1)
   if (populations eq 2) then sig2_r = sigma_r(r_sigdat,rp_nu,rp_M,rp_beta,Mpars,nupars2,betapars2)
   ;print,sig1_r 
...
;            total((sig2_r - sig_dat2)^2.)
   ...
   IF (ran LT fnewoverf) THEN BEGIN
      ...
      IF (testplot EQ 'yes') THEN BEGIN
         ...
         rplminsig = 0;
         rplmaxsig = max(r_sigdat)
         rpntssig = 500
         rplsig = dindgen(rpntssig) * (rplmaxsig-rplminsig)/double(rpntssig-1) + rplminsig
         nu1 = nu(rpl,rp_nu,nupars1)
         sig1 = sigma_r(rplsig,rp_nu,rp_M,rp_beta,Mpars,nupars1,betapars1)
         M = Mr(rpl,rp_M,Mpars)
         ...
         oplot,rpl,nu1,color=2
         
         plot,r_sigdat,sig_dat1,xrange=[0,rplmaxsig],yrange=[0,1.],/NODATA,$
              title='!6',xtitle='r(Rs)',ytitle='sigma_r1 []'
         oploterror,r_sigdat,sig_dat1,sig_dat_err1
         oplot,rplsig,sig1,color=2

         if (populations eq 2) then begin
            sig2 = sigma_r(rplsig,rp_nu,rp_M,rp_beta,Mpars,nupars2,betapars2)
            ...
            plot,r_sigdat,sig_dat2,xrange=[0,rplmaxsig],yrange=[0,1.],/NODATA,$
                 title='!6',xtitle='r(Rs)',ytitle='sigma_r2 []'
            oploterror,r_sigdat,sig_dat2,sig_dat_err2
            oplot,rplsig,sig2,color=2
         endif
         plot,r_Mdat,M_dat,xrange=[0,rplmax],yrange=[0,1.],/NODATA,$
