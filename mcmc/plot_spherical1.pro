...
IF (plotsim EQ 'yes') THEN BEGIN
   ...
   if (populations eq 2) then begin
      basename = dir+'/sphericalmethod_output/dual_unit_hern_'+whichsim+'/M'+Mno+'_nu'+nuno+'_beta'+betano+'/met_popboth_cprior_mirr_log_beta_mslope'
   ...
   IF (populations eq 1) then begin
      basename = dir+'/sphericalmethod_output/dual_unit_hern_'+whichsim+'/M'+Mno+'_nu'+nuno+'_beta'+betano+'/met_pop'+whichpop+'_cprior_mirr_log_beta_mslope'
...
npars = LONG(populations*nt_nu+nt_M+populations*nt_beta+1)
npars = LONG(npars)
niter = LONG(niter)
metarray = $
...
readcol,nufile1,r_sigdat,nu_dat1,nu_dat_err1
if (populations eq 2) then readcol,nufile2,r_dat,nu_dat2,nu_dat_err2
readcol,sigfile1,r_sigdat,sig_dat1,sig_dat_err1
if (populations eq 2) then readcol,sigfile2,r_dat,sig_dat2,sig_dat_err2
...
ENDIF ELSE BEGIN 
   ...
   Mslopepars = metarray(npars-1,burnin:niter-1)
ENDELSE 
...
FOR i=0L,rpnts-1 DO BEGIN 
   ...
   indexMslope = sort(Mslope(0:nn-1))
   ...
   FOR j=0L,np-1 DO BEGIN
      ...
      Mslope_p = Mslope(indexMslope(p(j)*(nn-1)))
   ENDFOR
ENDFOR
...
arraydump,basename+'_Mslope_p.txt',[[Mslope_p(0)],[Mslope_p(1)],[Mslope_p(2)],$
                                 [Mslope_p(3)],[Mslope_p(4)]],5
...
plot,xarr/1.e4,redchi,yrange=[0.1,100],/ylog,title='!6',$
...
maxrdat = max(r_Mdat) 
;compute M_r outside max(r_dat)
M_outer=dblarr(np,2)
r_outer=dblarr(2)
r_outer(0)=max(rpl)
r_outer(1)=2*max(rpl)
For i=0L,np-1 do begin
   M_outer(i,0)=M_rp(i,rpnts-1)
   M_outer(i,1)=M_rp(i,rpnts-1)+Mslope(i)*M_rp(i,rpnts-1)
endfor
...
FOR i=0L,np-1 DO BEGIN
   IF (i EQ 0) THEN BEGIN 
      plot,rpl,M_rp(i,*),title='!6',xtitle='r(Rs)',$
           ytitle='M!dr!n [M]',$
           yrange=[0,Mmax],xrange=[0,maxrdat],$
           linestyle = pstyle(i)
      oplot,r_outer,M_outer(i,*),linestyle=style(i)
...
;      oplot,[0,5],[sigs(0)+sigs(1),sigs(0)+sigs(1)],linestyle=1,color=6
   ...
   ENDIF ELSE $
      ...
      oplot,r_outer,M_outer(i,*),linestyle=pstyle(i)
ENDFOR
...
FOR i=0L,np-1 DO BEGIN
   IF (i EQ 0) THEN BEGIN
      plot,rpl,sig1_rp(i,*),title='!6',xtitle='r(Rs)',$
           ...
           yrange=[ysigmin,ysigmax],xrange=[0,max(r_dat)],$
           linestyle = pstyle(i)
...
FOR i=0L,np-1 DO BEGIN
   IF (i EQ 0) THEN BEGIN
      plot,rpl,beta1_rp(i,*),title='!6',xtitle='r(Rs)',/ylog,$
           ytitle='!7m!6 [units]',yrange=[ybetamin,ybetamax],$
           xrange=[0,max(r_dat)],$
           linestyle = pstyle(i)
