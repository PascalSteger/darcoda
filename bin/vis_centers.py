...
with sod generated substructure positions.'''
...
if i != 6:
	...
	print "$: vis_centers.py xc yc zc rvir mmin"
...
xmi = xc-rvir; xma = xc+rvir
ymi = yc-rvir; yma = yc+rvir
zmi = zc-rvir; zma = zc+rvir
...
for line in ahf:
	if(line[0]=="#"):
		continue
	values=line.split()
	x = float(values[2]); #0 for halosp, 2
	y = float(values[3]); #1 for halosp, 3
	z = float(values[4]); #2 for halosp, 4

	xa=((x-xc)*zoom+0.5)*nx
	ya=(1-((y-yc)*zoom+0.5))*ny
	za=z
