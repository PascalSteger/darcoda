program sphericaloverdensity
  ...
  real(kind=8),dimension(:),allocatable::mp,tmp
...
subroutine tri_ngp(x,mp,pdens,isort,npart,nx,ny,nz,npart_new)
  ...
  real(kind=8),dimension(1:npart,1:3)::x
...
!  integer,dimension(1:npart)::structure
  ...
  real(kind=8),dimension(:),allocatable::m_sel
  real(kind=8),dimension(:,:),allocatable::x_sel
  ...
  real(kind=8)::dx,dy,dz,rayon,volume,overdens
  real(kind=8)::Mtotsel
