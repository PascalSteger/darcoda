program rho_cell
  !--------------------------------------------------------------------------
  ! read out density from amr/hydro cells, up to maximum level lma,
  ! exclude parent cells
  !--------------------------------------------------------------------------
  implicit none
  integer::ndim,i,j,k,twotondim,type=0
  ...
  real::boxlen
  real::t
  real::xxmin=0.0,xxmax=1.0,yymin=0.0,yymax=1.0,zzmin=0.0,zzmax=1.0,dx,dx2
  ...
  real(KIND=8)::dkey,order_min,dmax,unit_d
  real(KIND=8)::xc=0.5,yc=0.5,zc=0.5,rc=0.001
  ...
  real(KIND=8),dimension(1:8,1:3)::xci
  ...
  nomfich=TRIM(repository)//'/hydro/hydro_'//TRIM(nchar)//'.out00001'
  ...
  read(10,*)!omega_b
  read(10,*)!unit_l
  read(10,'(A13,E23.15)')junk,unit_d!unit_d
  write(*,*)unit_d
  read(10,*)!unit_t

  read(10,*)!empty line

  read(10,'(A14,A80)')junk,ordering
  write(*,'(A14,A20)')junk,TRIM(ordering)
  read(10,*)
  ...
  xxmin=xc-rc ; xxmax=xc+rc
  yymin=yc-rc ; yymax=yc+rc
  zzmin=zc-rc ; zzmax=zc+rc
...
! ROM: comment the single follwing line for old stuff
     ...
     nomfich=TRIM(repository)//'/hydro/hydro_'//TRIM(nchar)//'.out'//TRIM(ncharcpu)
     ...
     do ilevel=1,lmax
        ...
        do ind=1,twotondim
           ...
           iy=(ind-1-4*iz)/2
           ix=(ind-1-2*iy-4*iz)
           xci(ind,1)=(dble(ix)-0.5D0)*dx
           xci(ind,2)=(dble(iy)-0.5D0)*dx
           xci(ind,3)=(dble(iz)-0.5D0)*dx
        end do
        ...
        if(ngrida>0)then
           ...
           allocate(var(1:ngrida,1:twotondim))
        ...
        do j=1,nboundary+ncpu
           ...
           if(ngridfile(j,ilevel)>0)then
              ...
              do ind=1,twotondim
                 do ivar=1,nvarh
                    if(ivar.ne.type.or.j.ne.icpu)then
                       read(11)
                    else
                       read(11)var(:,ind)
                    end if
        ...
        if(ngrida>0)then
           ...
           do ind=1,twotondim
              ! Compute cell center
              do i=1,ngrida
                 x(i,1)=(xg(i,1)+xci(ind,1)-xbound(1))
                 x(i,2)=(xg(i,2)+xci(ind,2)-xbound(2))
                 x(i,3)=(xg(i,3)+xci(ind,3)-xbound(3))
              end do
              ! Check if cell is refined
              do i=1,ngrida
                 ref(i)=son(i,ind)>0.and..not.ilevel<lmax
              end do
              ! Store data cube
              do i=1,ngrida
! exclude parent cells
!                 ok_cell= .not.ref(i).and. &
!                      & (x(i,1)-xc)**2+(x(i,2)-yc)**2+(x(i,3)-zc)**2<=rc**2
                 ok_cell = (x(i,1)-xc)**2+(x(i,2)-yc)**2+(x(i,3)-zc)**2<=rc**2
                 if(ok_cell)then
                    var(i,ind)=max(var(i,ind),1d-99)
                    write(20,999)x(i,1),x(i,2),x(i,3),dx,var(i,ind)
                 end if
              end do
  ...
  close(20)
  write(*,*)'file '//TRIM(outfich)//' written'

999 format(5(1pe12.4E4,1x))
...
contains
  ...
  subroutine read_params
    ...
    n = iargc()
    if (n < 4) then
       print *, 'usage: rho_cell  -inp  input_dir'
       ...
       print *, '                 [-rc rc]'
       print *, '                 [-lma lmax]'
       print *, '                 [-typ typ, 0 is default]'
       print *, ' typ gives: 0 density, (1,2,3) u,v,w , 4:pressure'
       print *, 'ex: rho_cell -inp output_00001 -out column.dat'// &
            &   ' -xc 0.5 -yc 0.5 -zc 0.5 -rc 0.25 -lma 21 -typ 0'
       print *, ' '
       stop
    end if
