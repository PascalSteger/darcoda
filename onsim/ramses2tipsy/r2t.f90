program ramses2tipsy
  ...
  logical::cosmo=.true.,star=.false.,sink=.false.,mhd=.false.,gas=.false.
  ...
  if ( .not. ok ) then
     print *,TRIM(nomfich)//' not found.'
     print *,' stopping because file was not found'
     stop
  endif
  ...
  if(partok)then
     ...
     do i=1,ncpu
        ...
        write(nn,'(I5.5)')i
        nomfich=TRIM(repository)//'/part/part_'//TRIM(nchar)//'.out'//nn
  ...
  read(10,*)
  read(10,'(A14,A80)')string,ordering
  ...
  if(TRIM(ordering).eq.'hilbert')then
     ...
     cpu_read=.false.
     do impi=1,ncpu
        read(10,'(I8,1X,E23.17,1X,E23.17)')i,bound_key(impi-1),bound_key(impi)
        print *,i,bound_key(impi-1),bound_key(impi)
     end do
  endif
  close(10)
  ...
  if(TRIM(ordering).eq.'hilbert')then
     print *,' ordering type is hilbert'
     ...
     do i=1,ndom
        ...
        print *,' dkey * order_min = ',dkey*order_min
        print *,' order_min+1.0D0 = ',order_min+1.0D0
        print *,' dkey * 1.0D0 = ',dkey*1.0D0
        print *,' (order_min+1.0D0)*dkey = ',(order_min+1.0D0)*dkey
        bounding_max(i)=(order_min+1.0D0)*dkey
        print *,' bounding(',i,') = (',bounding_min(i),',',bounding_max(i),')'
     end do

     cpu_min=0; cpu_max=128 ![PS] fix to get all files read in
     do impi=1,ncpu
        print *,' impi = ',impi
        do i=1,ndom
           ...
           print *,' cpu_min(',i,') = ',cpu_min(i),', cpu_max(',i,') = ',cpu_max(i)
        end do
     end do
     ...
     do i=1,ndom
        ...
        do j=cpu_min(i),cpu_max(i)
           print *,' i=',i,', j=',j,',ncpu_read=',ncpu_read
           if(.not. cpu_read(j))then
              ncpu_read=ncpu_read+1
              print *,'  increased ncpu_read to ',ncpu_read
