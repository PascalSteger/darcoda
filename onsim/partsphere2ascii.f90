program partsphere2ascii
  !--------------------------------------------------------------------------
  ! extract pos/vel/mass/id of DM particles inside sphere to ASCII format
  !--------------------------------------------------------------------------
  implicit none
  integer::ncpu,ndim,npart,i,k,icpu,ipos,nstar
  integer::ncpu2,npart2,ndim2,levelmin,levelmax
  integer::nx=0,ny=0,n_frw
  real(KIND=8)::mtot,time_tot,time_simu,weight,boxlen
  real(KIND=8)::xc=0.5,yc=0.5,zc=0.5,r=0.001
  real(kind=8)::dd2=0.0,rr2=0.0
  ...
  do k=1,ncpu!ncpu_read
     ...
     do i=1,npart2
        weight=1.
        dd2 = (x(i,1)-xc)**2 + (x(i,2)-yc)**2 + (x(i,3)-zc)**2
        rr2 = r**2
        ok_part=(dd2<rr2)
...
contains
  subroutine read_params
      ...
      n = iargc()
      if (n < 4) then
         print *, 'usage: partsphere2ascii -inp input_dir'
         ...
         print *, '                       [-fil filetype] '
         print *, '                       [-den filedens] '
         print *, 'ex: partsphere2ascii -inp output_00001 -out map.dat'// &
              &   ' -xc 0.45 -yc 0.55 -zc 0.34 -r 0.01'
         stop
      end if
