program part2map
  ...
  ! in work, problem: readin of age/metal for stars
  !--------------------------------------------------------------------------
  implicit none
  integer::ncpu,ndim,npart,i,j,k,icpu,ipos,nstar
  integer::ncpu2,npart2,ndim2,levelmin,levelmax,ilevel,iii
  integer::nx=0,ny=0,ix,iy,idim,jdim,ncpu_read,n_frw
  real(KIND=8)::mtot,ddx,ddy,time,time_tot,time_simu,weight,boxlen
  real(KIND=8)::xmin=0,xmax=1,ymin=0,ymax=1,zmin=0,zmax=1
  integer::imin,imax,jmin,jmax,kmin,kmax,lmin,npart_actual,nsink
  real(KIND=8)::xxmin,xxmax,yymin,yymax,dx,dy,deltax,massconv,vscale
  ...
  real(KIND=8),dimension(:)  ,allocatable::m,age,level,metal
  ...
  logical::ok,ok_part,periodic=.false.,star=.false.,ageweight=.true.,do_density=.false.
  integer::impi,ndom,bit_length,maxdom
  integer,dimension(1:8)::idom,jdom,kdom,cpu_min,cpu_max
  real(KIND=8),dimension(1:8)::bounding_min,bounding_max
  real(KIND=8)::dkey,order_min,dmax,pi
...
!     print *, "# FILE ",nomfich
  ...
  write(*,*)'#totally ',npart,' particles.'
  if(nstar>0)then
     if(star)then
        write(*,*)'#considered',nstar,' star particles.'
     else
        write(*,*)'#no actions done for',nstar,' star particles.'
     endif
  else
     write(*,*)'#nstar == 0'
  endif
...
!     write(*,*)'#Processing file '//TRIM(nomfich)
     read(1)ncpu2
     read(1)ndim2
     read(1)npart2!#particles in this snapshot file
     read(1)!localseed
     read(1)!nstar_tot
     read(1)!mstar_tot
     read(1)!mstar_lost
     ...
     write(*,*)'first entry:',x(1,1),x(1,2),x(1,3)
     write(*,*)'first entry:',x(2,1),x(2,2),x(2,3)
     write(*,*)'first entry:',x(3,1),x(3,2),x(3,3)
     write(*,*)'first entry:',x(4,1),x(4,2),x(4,3)
     write(*,*)'first entry:',x(5,1),x(5,2),x(5,3)
     write(*,*)'last entry:',x(npart2,1),x(npart2,2),x(npart2,3)
     ...
     write(*,*)'first entry:',v(1,1),v(1,2),v(1,3)
     write(*,*)'last entry:',v(npart2,1),v(npart2,2),v(npart2,3)

     write(*,*)'#Read mass'
     read(1)m
     write(*,*)'first entry:',m(1)
     write(*,*)'last entry:',m(npart2)

     write(*,*)'#Read identity'
     allocate(id_dm(1:npart2))
     read(1)id_dm
     write(*,*)'first entry:',id_dm(1)
     write(*,*)'last entry:',id_dm(npart2)

     write(*,*)'#Read level'
     allocate(level(1:npart2))
     read(1)level
     write(*,*)'first entry:',level(1)
     write(*,*)'last entry:',level(npart2)

     if(nstar>0)then
        write(*,*)'#allocating star age,metal arrays with',npart2,'entries'
        allocate(age(1:npart2))
        allocate(metal(1:npart2))

        write(*,*)'#read age'
        read(1)age
        write(*,*)'#read metallicity'
        read(1)metal ! Skip identity
     ...
     if(do_density)then
        ...
        do i=1,npart2
           ...
           if(ok_part)then
              ...
              ix=ddx
              iy=ddy
              map(ix,iy)=MAX(map(ix,iy),density(npart_actual))
           endif
        end do
     else
        do i=1,npart2
...
!                 write(*,*)'ok_part:',ok_part
           ...
           if(ok_part)then
              npart_actual=npart_actual+1
              if(star)then 
                print "(4(1pe15.8,1X),I8,1X,5(1pe15.8,1X))", m(i)*unit_d*unit_l**3/2d33,x(i,1),x(i,2),x(i,3),id_star(i),&
v(i,1)*vscale,v(i,2)*vscale,v(i,3)*vscale,metal(i),age(i)
!                 print "(6(1pe15.8,1X),I8,1X,3(1pe15.8,1X))", &
!                     m(i)*unit_d*unit_l**3/2d33,&
!                     x(i,1),x(i,2),x(i,3),&
!                     metal(i),age(i),id_star(i),&
!                     v(i,1)*vscale,v(i,2)*vscale,v(i,3)*vscale
     ...
     deallocate(x,m,v,id_dm,level)
     if(nstar>0) deallocate(age,metal,id_star)
  end do
