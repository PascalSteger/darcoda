...
program ramses2gadget  
  ...
  real*8, parameter :: kpc_in_cm = 3.08d21 ! the factor is from ramses: don't use more digits!
  ...
  real(dp), dimension(:,:), allocatable :: gaspart_pos, ramsespart_pos, dmpart_pos, starpart_pos
  real(dp), dimension(:,:), allocatable :: gaspart_vel, ramsespart_vel, dmpart_vel, starpart_vel
  real(dp), dimension(:),   allocatable :: gaspart_m,   ramsespart_m,   dmpart_m,   starpart_m
  integer*4,dimension(:), allocatable :: gaspart_id,  ramsespart_id,  dmpart_id,  starpart_id
  real(dp), dimension(:),   allocatable :: gaspart_u
  real(dp), dimension(:),   allocatable :: ramsespart_age
  
  ! temporary variables for the particle data
  real(dp) :: x, y, z, vx, vy, vz, m, u, m_group
  integer*4 :: id
  ...
  integer*4 :: mpi_reduce_buffer, files_per_cpu
#endif
  ...
  gadget_velocity_unit        = 1d5 * sqrt(a) ! sqrt(a) factor: see GADGET user's guide
  ...
  ufactor          = (ramses_length_unit/ramses_time_unit)**2 / gadget_etherm_unit !only if gas
  ...
  do ifile = firstfile, lastfile
     ...
     !!! PREPARE AMR GRID !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        ! number and generating their id's
        ngaspart = 0
	...
	! [ RF ] note: in this branch neither ndmpart nor nstarpart are yet set correctly
     ...
     ! see GADGET user's guide to understand the structure of this header. it is quite simple.
