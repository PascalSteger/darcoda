program get_sphere_dm
  !-------------------------------------------------------------------
  ! extract particles (DM/stars) to ASCII
  !-------------------------------------------------------------------
  implicit none
  integer::ncpu,ndim,npart,i,j,k,icpu,ipos,iii
  integer::ncpu2,npart2,ndim2,levelmin,levelmax
  integer::nx=0,ny=0,ncpu_read,n_frw
  real*8::mtot,time,time_tot,time_simu,weight,boxlen
  real*8::xc,yc,zc,r, drx, dry, drz, dd2
  integer::npart_actual,nsink
  real*8::massconv,vscale
  ...
  real*8,dimension(:,:),allocatable::map,x,v
  real*8,dimension(:),allocatable::m,age,metal
  ...
  logical::ok,ok_part,periodic=.false.,ageweight=.true.
  integer::impi
  real*8::pi
  ...
  do k=1,ncpu!ncpu_read
     ...
     allocate(m(npart2))
     allocate(age(npart2))
     allocate(metal(npart2))
     allocate(id(npart2))
     allocate(level(npart2))
     allocate(x(npart2,ndim2))
     allocate(v(npart2,ndim2))
     ! Read position
     do i=1,ndim
       !print *,'# read x dim',i
        read(1)m
     ...
     do i=1,npart2
        ...
        ok_part=(dd2<r**2.and.metal(i)==0.0) ! exclude stars as well
        
        if(ok_part)then
           npart_actual=npart_actual+1
           !print "(I8,1X)", id(i)
           print "(4(1pe15.8,1X),I8,1X,3(1pe15.8,1X))", m(i)*unit_d*unit_l**3/2d33,x(i,1),x(i,2),x(i,3),id(i), &
                v(i,1)*vscale,v(i,2)*vscale,v(i,3)*vscale
           mtot=mtot+m(i)
        end if
     end do
     deallocate(x,m,v,id,level,age,metal)
  end do
...
contains
  ...
  subroutine read_params
      ...
      if (n < 4) then
         ...
         print *, '                 [-str flag] '
         print *, '                 [-fil filetype] '
         print *, 'ex: get_sphere -inp ../run/output_00270'// &
...
subroutine hilbert3d(x,y,z,order,bit_length,npoint)
  ...
  if(bit_length>bit_size(bit_length))then
     print *,'Maximum bit length=',bit_size(bit_length)
...
subroutine friedman(O_mat_0,O_vac_0,O_k_0,alpha,axp_min, &
  ...
  implicit none
  integer::ntable
  real*8::O_mat_0, O_vac_0, O_k_0
  real*8::alpha,axp_min,age_tot
  real*8,dimension(0:ntable)::axp_out,hexp_out,tau_out,t_out
  ! ######################################################!
  ...
  ! ######################################################!
  real*8::axp_tau, axp_t
  real*8::axp_tau_pre, axp_t_pre
  real*8::dadtau, dadt
  real*8::dtau,dt
  real*8::tau,t
...
!     print *,'The sum must be equal to 1.0, but '
!     print *,'O_mat_0+O_vac_0+O_k_0=',O_mat_0+O_vac_0+O_k_0
!     stop
!  end if
...
function dadtau(axp_tau,O_mat_0,O_vac_0,O_k_0) 
  real*8::dadtau,axp_tau,O_mat_0,O_vac_0,O_k_0
...
function dadt(axp_t,O_mat_0,O_vac_0,O_k_0)
  real*8::dadt,axp_t,O_mat_0,O_vac_0,O_k_0
  dadt   = (1.0D0/axp_t)* &
       &   ( O_mat_0 + &
