     ...
     if(periodic)then
        do i=1,npart2
           ...
           if(ok_part)then
              ...
              ix=ddx
              iy=ddy
              ddx=ddx-ix;           ddy=ddy-iy
              dex=1.0-ddx;          dey=1.0-ddy
              if(ix<0)ix=ix+nx
              if(ix>=nx)ix=ix-nx
              if(iy<0)iy=iy+ny
              if(iy>=ny)iy=iy-ny
              ...
              map(ix  ,iy  )=map(ix  ,iy  )+metal(i)*dex*dey
              map(ix  ,iyp1)=map(ix  ,iyp1)+metal(i)*dex*ddy
              map(ixp1,iy  )=map(ixp1,iy  )+metal(i)*ddx*dey
              map(ixp1,iyp1)=map(ixp1,iyp1)+metal(i)*ddx*ddy
              mtot=mtot+m(i)
           end if
        end do
     else
        do i=1,npart2
           ...
           if(ok_part)then
              ...
              if(ix>=0.and.ix<nx.and.iy>=0.and.iy<ny)then
                 map(ix  ,iy  )=map(ix  ,iy  )+metal(i)*dex*dey
                 map(ix+1,iy  )=map(ix+1,iy  )+metal(i)*ddx*dey
                 map(ix  ,iy+1)=map(ix  ,iy+1)+metal(i)*dex*ddy
                 map(ix+1,iy+1)=map(ix+1,iy+1)+metal(i)*ddx*ddy
                 mtot=mtot+m(i)
              endif
           end if
        end do
...
contains
  ...
  subroutine read_params
      ...
      n = iargc()
      if (n < 4) then
         print *, 'usage: metal2map -inp  input_dir'
         ...
         print *, 'ex: part2map -inp output_00001 -out map.dat'// &
              &   ' -dir z -xc 0.5 -yc 0.5 -zc 0.5 -rc 0.01'
         stop
      end if
