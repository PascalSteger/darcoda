program get_sphere_stars
  !-------------------------------------------------------------------
  ! count star particles in sphere around x,y,z
  !-------------------------------------------------------------------
  implicit none
  ...
  real(kind=8)::mtot,time_tot,time_simu,weight,boxlen
  real(kind=8)::xc,yc,zc,rc, drx, dry, drz, dd2
  integer::nstar_tot,npart_actual,nsink
  real(kind=8)::mstar_tot,massconv,vscale
  ...
  !print *,'#Age simu=',(time_tot+time_simu)/(h0*1d5/3.08d24)/(365.*24.*3600.*1d9)
  ...
  if(nstar>0)then
     !print *,'#Found ',nstar,' stars totally.'
  else
  ...
  do k=1,ncpu!ncpu_read
     ...
     deallocate(x,m,v,id,level)
     if(nstar>0) deallocate(age,metal)
  end do
  !print *,'#Total mass=',mtot
  print *,1.0/aexp-1.0,nstar_tot,mstar_tot,npart_actual,mtot
...
contains
  ...
  subroutine read_params
      ...
      n = iargc()
      if (n < 4) then
         print *, 'usage: count_stars -inp input_dir'
         print *, '                 [-xc  xcenter [Mpc/h]] '
         print *, '                 [-yc  ycenter [Mpc/h]] '
         print *, '                 [-zc  zcenter [Mpc/h]] '
         print *, '                 [-rc  radius  [Mpc/h]] '
         print *, 'ex: get_sphere_stars -inp ../run/output_00270'// &
              &   ' -xc 0.5 -yc 0.5 -zc 0.5 -rc 0.1 > sample.dat'
         stop
      end if
