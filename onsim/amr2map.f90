...
! ROM: comment the single follwing line for old stuff
     ...
     do ilevel=1,lmax
        ...
        do ind=1,twotondim
           ...
           iy=(ind-1-4*iz)/2
           ix=(ind-1-2*iy-4*iz)
           xda(ind,1)=(dble(ix)-0.5D0)*dx
           xda(ind,2)=(dble(iy)-0.5D0)*dx
           xda(ind,3)=(dble(iz)-0.5D0)*dx
        end do
        ...
        if(ngrida>0)then
           ...
           do ind=1,twotondim
              ...
              ! Compute cell center
              do i=1,ngrida
                 x(i,1)=(xg(i,1)+xda(ind,1)-xbound(1))
                 x(i,2)=(xg(i,2)+xda(ind,2)-xbound(2))
                 if(ndim>2)x(i,3)=(xg(i,3)+xda(ind,3)-xbound(3))
              end do
...
!             allocate(var(1:ngrida,1:twotondim,1:nvarh))
              if(ilevel.eq.lmax)then
                 do i=1,ngrida
                    write(30, "(2E15.8)") rho(i), var(i,ind,1)
                 end do
              end if
              ...
              do i=1,ngrida
                 ...
                 if(ok_cell)then
                    ...
                    if(    ix>=grid(ilevel)%imin.and.&
                       ...
                       if(do_max)then
                          grid(ilevel)%map(ix,iy)=max(grid(ilevel)%map(ix,iy),rho(i))
                       else
                          grid(ilevel)%map(ix,iy)=grid(ilevel)%map(ix,iy)+rho(i)*dxline*weight/(zzmax-zzmin)
                       endif
                    endif
                 end if
              end do
