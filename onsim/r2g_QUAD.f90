  ...
  ! The original purpose is to create data files readable by the AMIGA 
  ! Halo Finder (AHF); but this tool can also be used for other purposes 
  ! where GADGET-format is handy.
  ! This program reads in data in QUAD format
  !                                                                   
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!    
  ...
  real*8            :: omega_m, omega_l, H0, h, a, gamma, boxsize
  
  ! unit conversion factors and parameters needed for that
  real*8            :: ramses_length_unit, ramses_dens_unit, ramses_time_unit
  real*8            :: gadget_length_unit, gadget_mass_unit, gadget_velocity_unit, &
                                & gadget_etherm_unit
  real*8            :: xfactor, vfactor, mfactor, ufactor
  real*8, parameter :: kpc_in_cm = 3.08d21 ! the factor is from ramses: dont use more digits!
  real*8, parameter :: Msun_in_cm = 1.9891d33
  ...
  integer*4 :: npart, ngaspart, nramsespart, ndmpart, nstarpart, nsink
  integer*4 :: ngaspart_total, ndmpart_total, nstarpart_total
  real*8, dimension(:,:), allocatable :: gaspart_pos, ramsespart_pos, dmpart_pos, starpart_pos
  real*8, dimension(:,:), allocatable :: gaspart_vel, ramsespart_vel, dmpart_vel, starpart_vel
  real*8, dimension(:),   allocatable :: gaspart_m,   ramsespart_m,   dmpart_m,   starpart_m
  integer*4,dimension(:), allocatable :: gaspart_id,  ramsespart_id,  dmpart_id,  starpart_id
  real*8, dimension(:),   allocatable :: gaspart_u
  real*8, dimension(:),   allocatable :: ramsespart_age
  
  ! temporary variables for the particle data
  real*8 :: x, y, z, vx, vy, vz, m, u, m_group
  integer*4 :: id
  ...
  integer*4,parameter :: gadget_fillheader(15) = 0
  integer*4           :: flagsfr, ngaspart_dummy, ndmpart_dummy, nstarpart_dummy
  integer*4           :: size
#ifdef MPI
  ! variables only needed if the code is run with mpi
  integer*4 :: mpi_reduce_buffer, files_per_cpu, mpi_ierr
#endif
  ...
  if (gas) then 
     amr_filename    = trim(dir_name) // '/amr/amr_'    // trim(dir_number) // '.out00001'
     hydro_filename  = trim(dir_name) // '/hydro/hydro_'  // trim(dir_number) // '.out00001'
  ...
  !!! START THE LOOP OVER SNAPSHOT FILES/CPUS !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  do ifile = firstfile, lastfile
     ...
     part_filename =   trim(dir_name) // '/part/part_'  // trim(dir_number) // '.out' // trim(suffix)
     info_filename =   trim(dir_name) // '/info_'  // trim(dir_number) // '.txt'
     amr_filename =    trim(dir_name) // '/amr/amr_'   // trim(dir_number) // '.out' // trim(suffix)
     hydro_filename =  trim(dir_name) // '/hydro/hydro_' // trim(dir_number) // '.out' // trim(suffix)
     output_filename = trim(dir_name) // '/r2g/r2g' // &
     ...
     !!! TODO: check with newest version of Ramses code
	...
	! [ RF ] note: in this branch neither ndmpart nor nstarpart are yet set correctly
     ...
     write(output_file) 'HEAD', 264_4 ! [PS: = 256+8 for unformatted, in 4 byte]
     ...
     if ((gas.eqv..false.).or.(nstarpart==0)) then ! [ RF ]
     ...
     if (gas) then
	   ...
	   ! write header block:
           ...
           write(output_file) (gaspart_pos(:,col),col=1,ngaspart), &
                & (dmpart_pos(:,col),col=1,ndmpart), &
                & (starpart_pos(:,col),col=1,nstarpart)
  
           ! write particle velocities
           size = 3*8*npart+8
           print *,'VEL size:',size
           write(output_file) 'VEL ', size
           write(output_file) (gaspart_vel(:,col),col=1,ngaspart), &
                & (dmpart_vel(:,col),col=1,ndmpart), &
                &(starpart_vel(:,col),col=1,nstarpart)
  
           ! write particle ids
           write(output_file) 'ID  ', 4*npart+8
           write(output_file) (gaspart_id(col),col=1,ngaspart), &
                & (dmpart_id(col),col=1,ndmpart), &
                & (starpart_id(col),col=1,nstarpart)
        
           ! write particle masses
           write(output_file) 'MASS', 8*npart+8
           write(output_file) (gaspart_m(col),col=1,ngaspart), &
                & (dmpart_m(col),col=1,ndmpart), &
                & (starpart_m(col),col=1,nstarpart)
  
           ! write gas particle thermal energies
           write(output_file) 'U   ', 8*ngaspart+8
           write(output_file) (gaspart_u(col),col=1,ngaspart)
...
#endif
  write(*,*)'ngaspart_total=',ngaspart_total,' ndmpart_total=',ndmpart_total,' nstarpart_total=',nstarpart_total
  ...
  if (icpu == 0) then
     ...
     write(*,*) 'ngaspart_total=',ngaspart_total, 'ndmpart_total=',ndmpart_total, & ! [ PS ]
          &          ' nstarpart_total=',nstarpart_total                            ! [ PS ]
...
!     print *,'size of dp:', sizeof(real*8)
