program get_temp
  ...
  integer::ndim,i,j,k,twotondim,type=0,domax=0
  integer::ivar,ncpu,lmax=0,levelmin
  integer::nx,ny,nz,ilevel,iidim,idim,jdim,kdim
  integer::nlevelmax
  integer::ind,ipos,ngrida
  integer::ngridmax,icpu,ncpu_read
  real::boxlen
  real*8::t,unit_l,unit_t,unit_d,unit_v
  real(dp)::gamma
  ...
  real(dp)::dkey,order_min,dmax,dxline
  real(dp)::xmin=0,xmax=1,ymin=0,ymax=1,zmin=0,zmax=1
  real(dp)::xs=0.5,ys=0.5,zs=0.5,rs=0.5
  real(dp)::xxmin,xxmax,yymin,yymax,zzmin,zzmax,dx
  ...
  read(10,*)!omega_k
  read(10,*)!omega_b
  read(10,'(A13,E23.15)')junk,unit_l
  print *,'unit_l = ',unit_l
  read(10,'(A13,E23.15)')junk,unit_d
  print *,'unit_d = ',unit_d
  read(10,'(A13,E23.15)')junk,unit_t
  print *,'unit_t = ',unit_t
  ...
  ! SELF: use "new" for explicit char representation
  nomfich=TRIM(outfich)
  open(30, file=nomfich, action="write")
  ...
  do k=1,ncpu_read
     ...
     open(unit=10,file=nomfich,form='unformatted')
     print *,'#Processing file '//TRIM(nomfich)
     do i=1,21
        read(10)
     end do
     print *,'skipped amr header'
     ! Read grid numbers
     read(10)ngridlevel
...
! ROM: comment the single follwing line for old stuff
     ...
     do i=1,3
        read(10)!coarse level, son, flag
     end do
     print *,'amr input done'
     ! Open HYDRO file and skip header
     nomfich=TRIM(repository)//'/hydro/hydro_'//TRIM(nchar)//'.out'//TRIM(ncharcpu)
     open(unit=11,file=nomfich,form='unformatted')
     read(11)!ncpu
     read(11)nvarh
     read(11)!ndim
     read(11)!nlevelmax
        ...
        if(ngrida>0)then
           ...
           do ind=1,twotondim
              ...
              case (15)
                 rho = var(:,ind,15)              
              case (16) !ekin
                 rho = 0.5*var(:,ind,1)**2*var(:,ind,1)
                 rho = rho+0.5*var(:,ind,2)**2*var(:,ind,1)
                 rho = rho+0.5*var(:,ind,3)**2*var(:,ind,1)
              case (17) !E_internal, U
                 rho = var(:,ind,5)/(gamma-1)
                 rho = rho+0.5*var(:,ind,2)**2*var(:,ind,1)
                 rho = rho+0.5*var(:,ind,3)**2*var(:,ind,1)
                 rho = rho+0.5*var(:,ind,4)**2*var(:,ind,1)
              case (18) !propto temperature, factor 2m_H/3R needed
                 rho = var(:,ind,5)/(gamma-1) !p/(gamma-1)
                 rho = rho/var(:,ind,1)       !p/(gamma-1)/rho
                 rho = rho*unit_v**2  !in physical units
                 rho = rho*1.132E-8!*1.5633D-24/1.381D-16 !*mH/kB in cgs
              end select

              if(.not.ref(i))then
                 do i=1,ngrida
                    write(30, "(2E15.8)") rho(i), var(i,ind,1)*(5.55d-25/1.67d-24)
                    if(rho(i)>maxrho)then
                       print *,rho(i)
                       maxrho=rho(i)
                    end if
                 end do
...
contains
  ...
  subroutine read_params
    ...
    if (n < 4) then
       ...
       print *, '                 [-typ type] '
       print *, '                 [-max maxi] '
       print *, 'ex: get_temp -inp output_00001 -out sample.dat'// &
            &   ' -xs 0.5 -ys 0.5 -zs 0.5 -rs 0.01 -lma 12'
       print *, ' '
