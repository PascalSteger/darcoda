...
program ramses2gadget  
  ...
  real(kind=8), parameter :: kpc_in_cm = 3.08d21 ! the factor is from ramses: don't use more digits!
  ...
  integer (kind=4) :: ngaspart_total, ndmpart_total, nstarpart_total, nramsespart_total
...
#else
#ifdef OPENMP
  ! variables only needed for openmp
   integer(kind=4)         :: files_per_cpu  
   integer(kind=4)         :: auxg,auxd,auxs,auxr
   integer :: omp_get_thread_num,omp_get_num_threads
   integer(kind=4)         :: offset_id
   auxg=0
   auxd=0
   auxs=0
   auxr=0
...
#else
#ifdef OPENMP
  !$omp parallel default(private) shared(auxg,auxd,auxs,auxr,offset_id,gadget_fillheader,kpc_in_cm,Msun_in_cm)
  ...
  !write(*,*) 'THIS IS CPU',icpu,' of ',ncpus,' CPUs'
...
!$omp barrier
  ...
  ! >> [ RF ]
  ! Open all part files and get total number of particles
  ! needed to give the created gas particles a sensible ID
  do ifile = firstfile, lastfile
     ...
     call inquire_file(part_filename,icpu)
     ...
     nramsespart_total = nramsespart_total + nramsespart
  enddo   
  ! << [ RF ]

#ifdef MPI 
  call MPI_ALLREDUCE(nramsespart_total,mpi_reduce_buffer, &
     & 1,MPI_INTEGER,MPI_SUM,MPI_COMM_WORLD,mpi_ierr)
  nramsespart_total = mpi_reduce_buffer
#endif 
#ifdef OPENMP  
  !$omp critical
  auxr = auxr + nramsespart_total
  !$omp end critical
  !$omp barrier 
  nramsespart_total = auxr
  if (icpu==0) write(*,*)'total No of star+dark particles = ',nramsespart_total
#endif  
  ...
  ! write(*,*) 'Loop from ',firstfile,' to ',lastfile   
  ...
  do ifile = firstfile, lastfile
     ...
     call inquire_file(part_filename,icpu)
     call inquire_file(info_filename,icpu)
     if (gas) then
        call inquire_file(amr_filename,icpu)
        call inquire_file(hydro_filename,icpu)
     end if
        ...
        do ilevel=1,nlevelmax
           ...
           if (ngrida>0) then
              ...
              do i=1,ngrida
		       ...
		       ! [ RF ]
		       id = nramsespart_total + (ngaspart*(nfiles-1))+ifile
#else 
                       ! [ RF ] in the serial or OpenMP case we can use a global counter for the ids
!#ifdef OPENMP
   !$omp critical  
!#endif		       		      
		       offset_id = offset_id + 1
		       id =  nramsespart_total + offset_id;			       	      
!#ifdef OPENMP
   !$omp end critical  
!#endif			       
  ...
  ! compute total number of stars etc in the OPEN-MP framework
  ! and end the parallel region
#ifdef OPENMP    
  !$omp critical
  auxg = auxg + ngaspart_total
  auxd = auxd + ndmpart_total
  auxs = auxs + nstarpart_total
  !$omp end critical  
  !$omp barrier 
  ngaspart_total = auxg
  ndmpart_total = auxd
  nstarpart_total=auxs   
  if (icpu==0) write(*,*)'ngaspart_total=',ngaspart_total,' ndmpart_total=',ndmpart_total,' nstarpart_total=',nstarpart_total 
#endif  
...
#else
  write(*,*) 'Compiled without MPI / OPENMP'
#endif
#endif
#ifdef OPENMP
  !$omp end critical
...
subroutine inquire_file(filename,icpu)
...
subroutine terminate(ierr, icpu)   
   ...
   if (icpu==0) write (*,'(A,I3,A)') 'Terminating (Error code', ierr, ').'
