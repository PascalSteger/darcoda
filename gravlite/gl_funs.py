...
## Get new parameters in the MCMC. No checking for unphysical values is performed.
#  @brief Get new parameters in the MCMC.
#  @see gravlite
#  @param testParameter String: Input Parameter is a string
#  @return Params Returns the new parameter class, gp.parst.
def get_new_parameters():
    ranarr = Params(0)

    # change parameters in one of the following ways:
    # ranarr.wiggle_dens() # if we want to wiggle only 
    #                         the density/mass/surfden array
    # ranarr.wigglebin()   # if we want to wiggle only 
    #                         the first bin at a time
    # ranarr.wigglepar()   # if we want to wiggle only 
    #                         one parameter at a time
    # ranarr.wiggle_delta()

    ranarr.setuniformrandom()   # wiggle all parameters as one

    # TODO: check outcome below. was used when working with adaptstepwait=1
    # if gp.initphase:
    #     ranarr.scale_prop_chi2()    # change proportional to error on chi2
    
    ranarr.mul(gp.parstep)
    ranarr.add(gp.pars) # add old accepted value, 
                                        # not possibly bad intermediate parst
    gp.parst.assign(ranarr)

    # keep deltaprior only during init phase, if set:
    if gp.deltaprior and gp.initphase:  # TODO: rename
        if gp.investigate == 'walker':           
            delta1,delta2 = betawalker(gp.xipol) # [1]
            gp.parst.set_delta([phys.invdelta(delta1),\
                                phys.invdelta(delta2)])
        else:
            gp.parst.set_delta([phys.invdelta(gp.delta0),\
                                phys.invdelta(gp.delta0)])
    return gp.parst
...
## Calculate M / nu / sigma values for disc case
...
## calculate likelihood from \chi^2
def calc_likelihood():
    if gp.uselike:
        # Errors:
        if gp.adderrors:
...
## determined 'reduced chi2'
# @param model
# @param data
# @param sig spread
# @param dof Degrees Of Freedomb
...
# @param n counter from main gravlite call
def accept_reject(n):
