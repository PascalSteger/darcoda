    ...
    for pop in np.arange(1, gp.pops+1):  # [1, 2, ..., gp.pops]
        ...
        if gp.chi2_Sig_converged:
            ...
            if gp.checksig:
                ...
                nr = -gh.deriv(np.log(anrho), np.log(gp.xepol))
                dn = gh.deriv(nr, np.log(gp.xepol))
                ...
                # check we get the right betastar profile again
                # betapar = popt4
