    ...
    for pop in np.arange(1, gp.pops+1):  # [1, 2, ..., gp.pops]
        ...
        if gp.chi2_Sig_converged:
            ...
            if gp.checksig:
                ...
                drho = -gh.deriv(np.log(anrho), np.log(gp.xepol))
                rhopar = np.hstack([rhopar_half, drho[0], drho, drho[-1]])
                rhostarpar = 0.0*rhopar
                MtoL = 0.0
                nupar = 1.*rhopar
                popt1 = np.array([ 0.24119257,  1.1770652 ])
                popt2 = np.array([ 0.03888825,  3.26930884, -2.48453452])
                popt3 = np.array([-0.01099247,  4.85755277, -7.62371943,  3.80946628])
                popt4 = np.array([-0.01546195,  5.18942424, -9.69433965,  7.52934688, -2.00699626])
                betapar = popt3
                pdb.set_trace()
