...
nuanf  = ga.rho_anf(r0, 1, 1) # 1/MtoL
Siganf = ga.Sig_anf(r0, 1, 1) # 1/MtoL
...
loglog(r0, nuanf, 'b.-')
nudatnu = glp.Rho_INT_rho(rfine, Sigdatnu, gp)
loglog(rfine, nudatnu, 'r.-', lw=0.5)
#nudatnuold = glp.Rho_INT_rho_buggy(rfine, Sigdatnu, gp) #negative found
#loglog(rfine, nudatnuold, 'g.-')
ipdb.set_trace()

dummy, nudatnu, nuerrnu, Mrdatnu = glp.Rho_NORM_rho(rfine, Sigdatnu, Sigerrnu, gp)
...
Sigdatproj_coarse = glp.rho_INTIPOL_Rho(r0, nuanf, gp)
Sigdatproj_fine   = glp.rho_INTIPOL_Rho(rfine, nudatnu, gp)
...
rhonu  = ga.rho_anf(rfine, 1, 1)
nunu   = ga.rho_anf(rfine, 1, 1/MtoL)
...
sigr2anf = ga.sigr2_anf(rfine, 1, 1, G1)
...
surfden = glp.rho_INT_Rho(rfine, nunu)
siglos = np.sqrt(sigl2s/surfden[:-gp.nexp])

elapsed_time = time.time() - start_time
print("elapsed time: "+str(elapsed_time))

surfdenanf = ga.Sig_anf(rfine, 1, 1/MtoL)
rfine = rfine[:-gp.nexp]
siglanf = ga.sig_los_anf(rfine, 1, 1, G1)
