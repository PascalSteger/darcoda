...
class Params():
    def __init__(self, timestamp = ''):
        ...
        self.case = 9 # gaia models (1..8) Walker (0..2,4,5; use 1, 2)
                      # triax (0-3:core, 4-8:cusp)
        ...
        if self.investigate == 'discmock' or \
           self.investigate == 'discsim':
            self.geom = 'disc'
            self.ndim = 1+self.nrho + self.pops*self.nepol + \
                        self.pops*self.nbeta # +1 for norm

        else:
            self.geom = 'sphere'
            self.ndim = self.nrho + (self.pops+1)*self.nepol + \
        ...
        self.maxbetaslope = 2   # linear (and 2nd..order) max slope
                                  # of beta*
