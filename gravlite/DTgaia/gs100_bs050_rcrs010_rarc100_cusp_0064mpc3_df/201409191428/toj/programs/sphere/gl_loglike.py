...
def geom_loglike(cube, ndim, nparams, gp):
    ...
    if gp.checksig:
        rho0check, rho1check, rho2check = ga.rho_walk(gp.xepol, gp)
        beta1check, beta2check = beta_walk(gp.xepol, gp)
    ...
    for pop in np.arange(1, gp.pops+1):  # [1, 2, ..., gp.pops]
        ...
        tmp_profs.set_prof('nrnu', 1.*nupar[1+1+gp.nexp:-gp.nexp-1], pop, gp)
        nu = phys.rho(gp.xepol, nupar, pop, gp)
        tmp_profs.set_prof('nu', nu[gp.nexp:-gp.nexp], pop, gp)
        off += gp.nrho

        betapar = np.array(cube[off:off+gp.nbeta])
        off += gp.nbeta

        Signu = glp.rho_INTIPOL_Rho(gp.xepol, nu, gp) # [Munit/pc^2]
        Sig = gh.linipollog(gp.xepol, Signu, gp.xipol)
