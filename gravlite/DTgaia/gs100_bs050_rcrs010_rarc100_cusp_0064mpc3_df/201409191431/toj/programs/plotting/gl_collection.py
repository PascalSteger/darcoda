...
class ProfileCollection():
    ...
    def set_analytic(self, x0, gp):
        ...
        if gp.investigate == 'gaia':
            ...
            self.analytic.x0 = r0
            rhoanalytic = ga.rhotot_gaia(r0, gp)
            self.analytic.set_prof('rho', rhoanalytic, 0, gp)
            Manalytic = rho_SUM_Mr(r0, rhoanalytic)
            self.analytic.set_prof('M', Manalytic, 0, gp)            
            self.analytic.set_prof('nr', ga.nr3Dtot_gaia(r0, gp), 0, gp)
            for pop in range(1, gp.pops+1):
                # r0 scaling in pc, fine
                self.analytic.set_prof('beta', ga.beta_gaia(r0, gp), pop, gp)
                be = ga.beta_gaia(r0, gp)
                self.analytic.set_prof('betastar', be/(2.-be), pop, gp)
                analytic_3D = ga.nu3Dtot_gaia(r0, gp)
                Sig_an = rho_INT_Rho(r0, analytic_3D)
                Signorm = gh.ipol_rhalf_log(r0, analytic_3D, gp.Xscale[pop])
                self.analytic.set_prof('Sig', Sig_an / Signorm, pop, gp)
                # TODO: set nu, nrnu
                self.analytic.set_prof('sig', 0.*r0, pop, gp) # TODO: find analytical profile
        elif gp.investigate == 'walk':
            r0 = x0 # [pc], spherical case
            self.analytic.x0 = r0
            rhoanalytic = ga.rho_walk(r0, gp)[0]
            self.analytic.set_prof('rho', rhoanalytic, 0, gp)
            Manalytic = rho_SUM_Mr(r0, rhoanalytic)
            self.analytic.set_prof('M', Manalytic, 0, gp)
            self.analytic.set_prof('nr', ga.nr3Dtot_deriv_walk(r0, gp), 0, gp)
            # TODO: rho^* and Sigma^* profiles from rho_walk
            for pop in range(1, gp.pops+1):
                be = ga.beta_walk(r0, gp)[pop-1]
                self.analytic.set_prof('beta', be, pop, gp)
                self.analytic.set_prof('betastar', be/(2.-be), pop, gp)
                analytic_3D = ga.rho_walk(r0, gp)[pop]
                Sig_an = rho_INT_Rho(r0, analytic_3D)
                #Signorm = gh.ipol_rhalf_log(r0, Sig_an, gp.Xscale[pop])
                self.analytic.set_prof('Sig', Sig_an, pop, gp)
                self.analytic.set_prof('sig', 0.*r0-1, pop, gp) # TODO
        elif gp.investigate == 'triax':
            r0 = x0
            self.analytic.x0 = r0
            rhoanalytic = ga.rho_triax(r0, gp)[0]
            self.analytic.set_prof('rho', rhoanalytic, 0, gp)
            Manalytic = rho_SUM_Mr(r0, rhoanalytic)
            self.analytic.set_prof('M', Manalytic, 0, gp)
            self.analytic.set_prof('nr', ga.nr3Dtot_deriv_triax(r0, gp), 0, gp)
            # TODO: rho^* and Sigma^* profiles from rho_walk
            for pop in range(1, gp.pops+1):
                be = ga.beta_triax(r0, gp)[pop-1]
                self.analytic.set_prof('beta', be, pop, gp)
                self.analytic.set_prof('betastar', be/(2.-be), pop, gp)
                analytic_3D = ga.rho_triax(r0, gp)[pop]
                Sig_an = rho_INT_Rho(r0, analytic_3D)
                #Signorm = gh.ipol_rhalf_log(r0, Sig_an, gp.Xscale[pop])
                self.analytic.set_prof('Sig', Sig_an, pop, gp)
                self.analytic.set_prof('sig', 0.*r0-1, pop, gp) # TODO
    ...
    def write_all(self, basename, gp):
        ...
        self.write_prof(basename, 'nu', 1, gp)
        self.write_prof(basename, 'nunr', 1, gp)
        self.write_prof(basename, 'sig', 1, gp)
        if gp.pops == 2:
            self.write_prof(basename, 'betastar', 2, gp)
            self.write_prof(basename, 'beta', 2, gp)
            self.write_prof(basename, 'Sig', 2, gp)
            self.write_prof(basename, 'nu', 2, gp)
            self.write_prof(basename, 'nunr', 2, gp)
            self.write_prof(basename, 'sig', 2, gp)
    ...
    def plot_profile(self, basename, prof, pop, gp):
        ...
        if gp.investigate == 'walk' or gp.investigate == 'gaia':
            ax.plot(self.analytic.x0, self.analytic.get_prof(prof, pop),\
                    'b--', lw=2)
