...
poptj, pcovj = curve_fit(modelbetaj, x, y)

x = gp.xfine
y = analytic_beta(x)
ax1.plot(x, y, 'b.-', label='analytic')
ax1.axvline(xturn)
ax1.set_ylim([-0.2, 1.2])
ax1.set_xscale('log')
ax1.plot(x, modelbeta3(x, *popt3), alpha=0.8, label='$sigmoid, offsets$')
ax1.plot(x, phys.betastar(x, xturn, popt3, gp), 'r--', lw=2, label='phys.beta')
ax1.plot(x, modelbetaj(x, *poptj), alpha=0.8, label='justin')
...
ax2.plot(x, y-modelbetaj(x, *poptj), alpha=0.8)
