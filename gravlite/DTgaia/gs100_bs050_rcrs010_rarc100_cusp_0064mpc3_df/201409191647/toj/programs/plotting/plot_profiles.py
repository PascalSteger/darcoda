...
if __name__ == '__main__':
    ...
    gp.xipol = Radii * gp.Xscale[0]       # [pc] TODO check
    maxR = max(Radii) # [pc]
    Radii = np.hstack([Radii, 2*maxR, 4*maxR, 8*maxR])
    gp.xepol = Radii * gp.Xscale[0]       # [pc] TODO check
    ...
    pc.plot_profile(basename, 'nu', 1, gp)
    pc.plot_profile(basename, 'nrnu', 1, gp)
    pc.plot_profile(basename, 'sig', 1, gp)
    if gp.pops == 2:
        pc.plot_profile(basename, 'betastar', 2, gp)
        pc.plot_profile(basename, 'beta', 2, gp)
        pc.plot_profile(basename, 'Sig', 2, gp)
        pc.plot_profile(basename, 'nu', 2, gp)
        pc.plot_profile(basename, 'nrnu', 2, gp)
        pc.plot_profile(basename, 'sig', 2, gp)
