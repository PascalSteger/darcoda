...
def betastar(r0, r0turn, arr, gp):
    gh.sanitize_vector(arr, 4, -1e6, 1e6)
    gh.sanitize_scalar(r0turn, 1e-10, max(gp.xfine))

    s=np.log(r0/r0turn)
    betatmp = arr[0]/(1+np.exp(arr[1]*s+arr[2]))+arr[3]*np.ones(len(r0))
    return betatmp
## \fn betastar(r0, r0turn, arr, gp)
# calculate betastar from 4 parameters
# @param r0 radii [pc]
# @param r0turn turn-over radius, normally rhalf or half the max radius
# @param arr 4 parameters
# @param gp global parameters


def betastar_old(r0, r0turn, arr, gp):
