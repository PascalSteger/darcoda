    ...
    for pop in range(gpr.pops):
        ...
        for k in range(gpr.n):
            ...
            for i in range(gp.nipol):
                ...
                Sig_kin[i][k] = float(len(ind1))*totmass/Vol[i]
                # [Munit/rscale**2]
            ...
            zetaa[k] = gh.starred(Rbin, v4[:][k], Sigma, Ntot)
            v2denom = (gh.starred(Rbin, v2[:][k], Sigma, Ntot))**2
            zetaa[k] /= v2denom

            zetab[k] = gh.starred(Rbin, v4[:][k]*Rbin**2, Sigma, Ntot)
            zetab[k] /= v2denom
            zetab[k] /= (gh.starred(Rbin, Rbin, Sigma, Ntot))**2
        ...
        numedi = glp.Rho_INT_rho(Rbin*Rscalei, Sig0pc*P_dens, gp)
        numin  = glp.Rho_INT_rho(Rbin*Rscalei, Sig0pc*(P_dens-P_edens), gp)
        numax  = glp.Rho_INT_rho(Rbin*Rscalei, Sig0pc*(P_dens+P_edens), gp)
        ...
        for b in range(gp.nipol):
            siglos = np.sum(siglos[b])/gpr.n #[km/s]
            tpbb = np.sum(tpb[b])/float(gpr.n) #[1]
            if tpbb == 0:
                sigloserr = p_edvlos[b-1] #[km/s]
                # attention! uses last error
            else:
                sigloserr = siglos/np.sqrt(tpbb) #[km/s]
            p_dvlos[b] = siglos    #[km/s]
            p_edvlos[b]= sigloserr #[km/s]
