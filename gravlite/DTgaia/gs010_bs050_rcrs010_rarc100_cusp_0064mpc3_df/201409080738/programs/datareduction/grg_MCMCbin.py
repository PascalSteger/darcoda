...
def run(gp):
    ...
    for pop in range(gpr.pops):
        ...
        f_Sig, f_nu, f_mass, f_sig, f_kap, f_zeta = gfile.write_headers_2D(gp, pop)
        ...
        for k in range(gpr.n):
            ...
            for i in range(gp.nipol):
                ...
                Sigma[i][k] = float(len(sel))*totmass/Vol[i] # [Munit/Rscale0^2]
        ...
        # output density
        Sig0 = np.sum(Sigma[0])/(1.*gpr.n) # [Munit/Rscale^2]
        Sig0pc = Sig0/Rscale0**2              # [munis/pc^2]
        gfile.write_Sig_scale(gp.files.get_scale_file(pop), Sig0pc, totmass)
        ...
        for b in range(gp.nipol):
            Sig = np.sum(Sigma[b])/float(gpr.n) # [Munit/Rscale^2]
            tpbb   = np.sum(tpb[b])/float(gpr.n)       # [1]
            Sigerr = Sig/np.sqrt(tpbb)       # [Munit/Rscale^2]
            ...
            if(np.isnan(Sigerr)):
                ...
                P_dens[b] = Sig/Sig0   # [1]
                P_edens[b]= Sigerr/Sig0    # [1] #100/rbin would be artificial guess
        ...
        numedi = Rho_INT_rho(Rbin*Rscalei, Sig0pc*P_dens, gp)
        numin  = Rho_INT_rho(Rbin*Rscalei, Sig0pc*(P_dens-P_edens), gp)
        numax  = Rho_INT_rho(Rbin*Rscalei, Sig0pc*(P_dens+P_edens), gp)
        ...
        if gpr.showplots:
            ...
            Ri = Rbin * Rscalei
            Sig_dat = Sig0pc*P_dens
            Sig_err = Sig0pc*P_edens

            nu_dat = Rho_INT_rho(Ri, Sig0pc*P_dens, gp)
            loglog(Ri, nu_dat, 'r.-', label='deprojected data, assuming M_{part} = 1 M_\odot')
            ylabel(r'$\nu\qquad[\rm{Msun}/\rm{pc}^3]$')

            Sig_gaia = ga.Sig_gaia(Ri, gp)
            #loglog(Ri, Sig_gaia, 'b.-', label='analytic Sigma')

            rho_gaia, nu_gaia = ga.rho_gaia(Ri, gp)
            loglog(Ri, nu_gaia, 'b.-', label='analytic nu, self-corrected')
            loglog(Ri, rho_gaia, 'g.-', label='analytic rho')
            
            xlabel(r'$R\qquad[\rm{pc}]$')
            legend()
            pdb.set_trace()
