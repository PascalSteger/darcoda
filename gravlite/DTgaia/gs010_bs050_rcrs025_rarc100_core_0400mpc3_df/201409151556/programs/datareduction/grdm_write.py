...
def write_disc_output_files(Rbin, Binmin, Binmax, nudat, nuerr, Sig, Sigerr, Mdat, Merr, sig, sigerr, scales, gp):
    for pop in range(gp.pops+1):
        ...
        for b in range(gp.nipol):
            print(Rbin[b], Binmin[b], Binmax[b], \
                  Sig[pop][b], Sigerr[pop][b], file=f_Sig)
        f_Sig.close()
...
## \fn write_disc_output_files(Rbin, Binmin, Binmax, nudat, nuerr, Sig, Sigerr, Mdat, Merr, sig, sigerr, scales, gp)
...
def run(gp):
    ...
    sig_dat_err_bin1 = np.sqrt(sig_dat_bin1) # TODO: Poisson errors
    
    counts, dum = gh.bincount(z_dat1, binmax1)
    nu_dat_bin1 = counts/(binmax1-binmin1)
    nu_dat_err_bin1 = np.sqrt(nu_dat_bin1) # TODO
    ...
    Sig_dat_bin1 = np.cumsum(counts)/binmax1
    Sig_err_bin1 = np.sqrt(Sig_dat_bin1) # TODO

    Mr1 = np.cumsum(counts)
    Mrerr1 = np.sqrt(Mr1) # TODO

    scales=[[],[],[]]
    scales[1].append(z0) # [pc]
    scales[1].append(Sig_dat_bin1[0])
    scales[1].append(Mr1[-1])
    scales[1].append(nu_dat_bin1[np.argmin(np.abs(zth-z0))]) # store nu(r_scale)
    ...
    if gp.pops == 2:
        ...
        binmin2, binmax2, z_dat_bin2, sig_dat_bin2, count_bin2 = gh.binsmooth(z_dat2, vz_dat2, \
                                                                              zmin, zmax, gp.nipol, 0.)
        sig_dat_err_bin2 = np.sqrt(sig_dat_bin2) # TODO: Poissonian errors

        counts, dum = gh.bincount(z_dat2, binmax2)
        nu_dat_bin2 = counts/(binmax2-binmin2)
        nu_dat_err_bin2 = np.sqrt(nu_dat_bin2) # TODO

        Sig_dat_bin2 = np.cumsum(counts)/binmax2
        Sig_err_bin2 = np.sqrt(Sig_dat_bin2) # TODO

        Mr2 = np.cumsum(counts)
        Mrerr2 = np.sqrt(Mr2) # TODO
        
        scales[2].append(z02) # [pc]
        scales[2].append(Sig_dat_bin2[0])
        scales[2].append(Mr2[-1])
        scales[2].append(nu_dat_bin2[np.argmin(np.abs(zth-z02))])
    ...
    counts, dum = gh.bincount(z_dat0, binmax0)
    nu_dat_bin0 = counts/(binmax2-binmin0)
    nu_dat_err_bin0 = np.sqrt(nu_dat_bin0) # TODO

    Sig_dat_bin0 = np.cumsum(counts)/binmax0
    Sig_err_bin0 = np.sqrt(Sig_dat_bin0) # TODO

    Mr0 = np.cumsum(counts)
    Mrerr0 = np.sqrt(Mr0) # TODO
    
    renorm0 = max(nu_dat_bin0)
 
    xip = np.copy(z_dat_bin0)                        # [pc]
    Mr0   = K*xip/np.sqrt(xip**2.+D**2.) / (2.0*np.pi*gp.G1)
    Mrerr0   = np.sqrt(Mr0)

    scales[0].append(D)  # [pc]
    scales[0].append(Sig_dat_bin0[0])
    scales[0].append(Mr0[-1])
    scales[0].append(nu_dat_bin0[np.argmin(np.abs(zth-D))])
    ...
    Mr = []
    Mr.append(Mr0/scales[0][2])            # [Msun]
    Mr.append(Mr1/scales[1][2])
    if gp.pops == 2:
        Mr.append(Mr2/scales[2][2])
    ...
    Sig = []
    Sig.append(Sig_dat_bin0/scales[0][3])
    Sig.append(Sig_dat_bin1/scales[1][3])
    if gp.pops == 2:
        Sig.append(Sig_dat_bin2/scales[2][3])

    Sigerr = []
    Sigerr.append(Sig_dat_bin0/scales[0][3])
    Sigerr.append(Sig_dat_bin1/scales[1][3])
    if gp.pops == 2:
        Sigerr.append(Sig_dat_bin1/scales[2][3])

    sig = []
    sig.append(sig_dat_bin0/scales[0][3])     # [km/s]
    sig.append(sig_dat_bin1/scales[1][3])
    if gp.pops == 2:
        sig.append(sig_dat_bin2/scales[2][3])
