...
def ant_sigkaplos(r0, rhopar, rhostarpar, MtoL, nupar, betapar, pop, gp):
    ...
    clf()
    loglog(gp.xipol, gp.dat.nu, 'b.-')
    loglog(r0nu, ga.rho_gaia(r0nu, gp)[1], 'g.-')
    loglog(r0nu, nunu, 'r.-')
    pdb.set_trace()
    Signu  = glp.rho_param_INT_Sig(r0nu, nupar, pop, gp)
    clf()
    loglog(gp.xipol, gp.dat.Sig, 'b.-')
    loglog(r0nu, Signu, 'r.-')
    pdb.set_trace()
    ...
    beta_star1, r_DM, gamma_star1, r_star1, r_a1, gamma_DM, rho0 = gp.files.params
    clf()
    plot(gp.xfine, (np.log(gp.xfine**2+r_a1**2)-np.log(r_a1**2))/2, 'b.-')
    plot(r0nu, idnu, 'r.-')
    pdb.set_trace()
