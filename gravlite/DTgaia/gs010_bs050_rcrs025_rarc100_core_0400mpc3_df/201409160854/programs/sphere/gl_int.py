...
def ant_sigkaplos(r0, rhopar, rhostarpar, MtoL, nupar, betapar, pop, gp):
    ...
    #clf()
    #plot(r0nu, ga.beta_gaia(r0nu, gp)[1], 'b.-')
    #plot(r0nu, betanu, 'r.-')
    #pdb.set_trace()
    nunu   = phys.nu(r0nu, nupar, pop, gp)
    #clf()
    #loglog(gp.xipol, gp.dat.nu[pop], 'b.-')
    #loglog(r0nu, ga.rho_gaia(r0nu, gp)[pop], 'g.-')
    #loglog(r0nu, nunu, 'r.-')
    #pdb.set_trace()
    ...
    #pdb.set_trace()
    idnu   = ant_intbeta(r0nu, gp.x0turn, betapar, gp)+0.1 # TODO tweak of 0.1 into calculation
    #clf()
    #plot(r0nu, idnu, 'r.-')
    beta_star1, r_DM, gamma_star1, r_star1, r_a1, gamma_DM, rho0 = gp.files.params
    #plot(r0nu, 0.5*(np.log(r0nu**2+r_a1**2)-np.log(r_a1**2)), 'b.-')
    #pdb.set_trace()

    # integrate enclosed 3D mass from 3D density
    r0tmp = np.hstack([0.,r0nu])

    rhoint = 4.*np.pi*r0nu**2*nunu # TODO change to rhonu
    ...
    loglog(gp.xipol, gp.dat.Mr[pop], 'b.-')
    # TODO: calculate analytic values
    loglog(r0nu, Mrnu, 'r.-')
    pdb.set_trace()
