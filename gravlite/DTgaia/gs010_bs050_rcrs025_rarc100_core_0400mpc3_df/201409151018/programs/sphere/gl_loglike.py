    ...
    for pop in np.arange(1, gp.pops+1):  # [1, 2, ..., gp.pops]
        ...
        if gp.chi2_Sig_converged:
            ...
            if gp.checksig:
                ...
                #dn = gh.derivipol(nr, np.log(gp.xfine))
                dlr = np.hstack([nr[0], nr, nr[-1]])
                rhopar = np.hstack([rhopar_half, dlr])
                rhostarpar = 0.0*rhopar
                MtoL = 0.0
                betapar = np.array([-0.04231198,  0.95755322,  4.01786059, -7.99164511,  3.95797097])
                # we get the right betastar profile again
                # betapar = popt4
                #ba = ga.beta_gaia(gp.xfine, gp)[0]
                ...
                #tck = phys.nr(gp.xfine, dlr, 0, gp)
                #nr2 = -splev(gp.xfine, tck)
                #plot(gp.xfine, nr2, 'r.-')
                pdb.set_trace()
                ...
                dnnu = gh.derivipol(nrnu, np.log(gp.xfine))
                dlrnu = np.hstack([nrnu[0], dnnu, nrnu[-1]])
