...
def modelbeta5(r0, arr0, arr1, arr2, arr3, arr4, arr5):
    ...
    betatmp += arr3 * (r0/xturn)**3
    betatmp += arr4 * (r0/xturn)**4
    betatmp += arr5 * (r0/xturn)**5
    return betatmp
def modelbeta6(r0, arr0, arr1, arr2, arr3, arr4, arr5, arr6):
    betatmp = arr0 * (r0/xturn)**0
    betatmp += arr1 * (r0/xturn)**1
    betatmp += arr2 * (r0/xturn)**2
    betatmp += arr3 * (r0/xturn)**3
...
popt6, pcov6 = curve_fit(modelbeta6, x, y)
...
ax1.plot(x, modelbeta6(x, *popt6), alpha=0.8, label='$6th order$')

ax1.plot(gp.xepol, phys.betastar(gp.xepol, xturn, popt6, gp), 'r--', lw=2, label='phys.beta')
...
ax2.plot(x, y-modelbeta6(x, *popt6), alpha=0.8)
