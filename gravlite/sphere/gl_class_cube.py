...
class Cube:
    ...
    def convert_to_parameter_space(self, gp):
        ...
        pc[gp.nepol-1] = pc[gp.nepol-1] * gp.maxrhoslope
        if gp.monotonic:
            pc[gp.nepol-1] += pc[gp.nepol-2]
        pc[gp.nepol-1] = max(pc[gp.nepol-1], 3.)
        # ATTENTION bounded between 3 and gp.maxrhoslope, favors 3
        off = gp.nepol
        for pop in range(gp.pops):
            ...
            pc[off+1] = (pc[off+1]**2) * 2.999 # nu inner asymptote
            for i in range(2, gp.nepol-1):
                pc[off+i] = (pc[off+i]**2) * gp.maxnuslope   # see above
                if gp.monotonic:
                    pc[off+i] += pc[off+i-1]

            pc[off+gp.nepol-1] = pc[off+gp.nepol-1] * gp.maxnuslope
            if gp.monotonic:
                pc[off+gp.nepol-1] += pc[off+gp.nepol-2]
            pc[off+gp.nepol-1] = max(pc[off+gp.nepol-1], 3.)
            # ATTENTION: bounded between 3 and gp.maxnuslope, favors 3
            off += gp.nepol
            ...
            for i in range(gp.nbeta-1):
                pc[off] = ((pc[off]-0.5)*2.)*gp.maxbetaslope # if rising beta prior: delete -0.5
                off += 1
