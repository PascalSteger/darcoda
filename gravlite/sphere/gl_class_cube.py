...
def map_nr(pa, scale, width, prof, gp):
    ...
    if prof=='rho':
        ...
        pa[gp.nepol-1] = pa[gp.nepol-1]*(gp.maxrhoslope-3)+3
        if monotonic:
            pa[gp.nepol-1] = max(pa[gp.nepol-1], pa[gp.nepol-2])

    # finite mass prior: to bound between 3 and gp.maxrhoslope, favoring 3:
    # pa[gp.nepol-1] = max(pa[gp.nepol-1], 3.)
    return pa
## \fn map_nr(pa, scale, width, prof, gp)
...
# @param prof 'nu' or 'rho', using two sets of priors from gl_params
# @param gp global parameters


def map_nu(pa, gp):
    for i in range(gp.nipol):
        pa[i] = 10**(pa[i]*(gp.maxlognu-gp.minlognu)+minlognu)
    return pa
## \fn map_nu(pa, gp)
...
class Cube:
    ...
    def convert_to_parameter_space(self, gp):
        ...
        pc = self.cube
        # DM density
        tmp = map_nr(pc[0:gp.nepol], gp.rhohalf, gp.rhospread, 'rho', gp)
        for i in range(gp.nepol):
            pc[i] = tmp[i]
        off = gp.nepol

        # rho*
        tmp = map_nu(pc[off:off+gp.nepol], gp.dat.nuhalf[0], \
                     gp.nuspread[0], 'nu', gp)
        for i in range(gp.nepol):
            pc[off+i] = tmp[i]
        off += gp.nepol
        ...
        for pop in range(gp.pops): # nu1, nu2, ...
            tmp = map_nu(pc[off:off+gp.nepol], gp.dat.nuhalf[pop], \
                         gp.nuspread[pop], 'nu', gp)
            for i in range(gp.nepol):
                pc[off+i] = tmp[i]
            off += gp.nepol
