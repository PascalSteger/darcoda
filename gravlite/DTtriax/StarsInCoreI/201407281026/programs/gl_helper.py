...
def complete_nu(R0, Sigdat, Sigerr, Rnu):
    R0left = Rnu[Rnu<R0[0]]   # extension of radii to the left
    R0right = Rnu[Rnu>R0[-1]] # extension of radii to the right
    R0nu = Rnu[(Rnu>=R0[0]) * (Rnu<=R0[-1])] # [pc]

    # TODO: use powerlaw to smaller and higher radii
    Sigdatleft = np.exp(expol(R0, np.log(Sigdat), R0left, 'linear'))
    Sigerrleft = (Sigerr[0]/Sigdat[0])*Sigdatleft

    Sigdatnu = linipollog(R0, Sigdat, R0nu)
    Sigerrnu = linipollog(R0, Sigerr, R0nu)

    Sigdatright = np.exp(expol(R0, np.log(Sigdat), R0right, 'linear'))
...
def expol(xin, yin, xout, fn='multiquadratic'):
