...
def introduce_points_in_between(r0, gp):
    rmin = np.log(min(r0))
    rmax = np.log(max(r0))
    return np.logspace(rmin, rmax, gp.fine)
...
# return e^{-2 \int_r_min^r \beta(s)/s ds}
# @param r0 radius [pc]
# @param betapar [1] coefficients of beta representation
# @param gp global parameters


def ant_sigkaplos2surf(r0, rhopar, rhostarpar, nupar, betapar, pop, gp):
    ...
    nunu   = gh.ipollog(gp.xipol, nupar, r0nu) # interpolate points
    ...
    for i in range(len(r0nu)):
        ...
        sigr2nu[i] = TODO*gh.quadinflog(xint, yint, r0nu[i], np.inf, True)
        if sigr2nu[i] == np.inf:
            sigr2nu[i] = 1e-100
    ...
    sigl2s_out = np.exp(splev(r0, tck))[:-3]
...
## \fn ant_sigkaplos2surf(r0, rhopar, rhostarpar, nupar, betapar, pop, gp)
...
def ant_sigkaplos2surf_ipol(r0, rhopar, rhostarpar, nupar, betapar, pop, gp):
    ...
    betanu, dum = phys.beta(r0nu, betapar, gp)
