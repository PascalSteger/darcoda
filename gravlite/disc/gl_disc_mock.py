...
def disc_mock(gp):
    ...
    K   = 1.65                                # [TODO]
    F   = 1.65e-4                             # [TODO]
    C   = 17.**2.                             # [TODO] integration constant in sigma

    # trick to speed up things considerably, after first data generated, if gp.nipol > 50 (or other low number)
    ...
    sigzstar = gh.ipol(zth, sigzth, zstar) # > 0 ((IDL, Justin)) stellar velocity dispersion

    # assign [0,1] * maxsigma
    ran2 = npr.normal(size=int(gp.ntracer[1-1]))  # [1]
    vzstar = ran2 * sigzstar                      # [TODO], v_z<=sigma_z, same dimensionality as [km/s]

    # Add second population [thick-disc like]: 
    if gp.pops == 2:
        nu_zth2 = gp.ntracer[2-1]/gp.ntracer[1-1]*np.exp(-zth/z02) # no normalization to 1
        inti    = np.zeros(nth)
        for i in range(1, nth):
        ...
        zstarobs = np.hstack([zstar, zstar2])                  # concat pop1 and pop2 for all stars
    ...
    gp.dat.binmin = binmin0                          # TODO: include second pop for calc
    gp.dat.rbin   = xip
    ...
    # gp.Mmodel = (K*xip/np.sqrt(xip**2.+D**2.)+2.*F*xip) / (2.0*np.pi*gp.G1) # TODO: gl_analytic
    ...
    gp.dat.nudat.append(nu_dat_bin0)
    gp.dat.nuerr.append(nu_dat_err_bin0)
        
    gp.dat.sigdat.append(sig_dat_bin0)
    gp.dat.sigerr.append(sig_dat_err_bin0)

    gp.dat.nudat.append(nu_dat_bin)
    gp.dat.nuerr.append(nu_dat_err_bin)

    gp.dat.sigdat.append(sig_dat_bin)
    gp.dat.sigerr.append(sig_dat_err_bin)
    
    if gp.pops == 2:
        gp.dat.nudat2.append(nu_dat_bin2)
        gp.dat.nuerr2.append(nu_dat_err_bin2)

        gp.dat.sigdat2.append(sig_dat_bin2)
        gp.dat.sigerr2.append(sig_dat_err_bin2)
