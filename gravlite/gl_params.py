...
class Params():
    def __init__(self, timestamp = ''):
        ...
        self.nipol = 10   # IF CHANGED => set getnewdata = True to run data readout again
        ...
        self.monotonic = True           # mono-prior on rho(z) and nu(z)

        self.xipol = np.array([])
        self.xepol = np.array([])

        self.maxR = 10.           # [Rscale], max range in radial bins
        self.maxrhoslope  = 2.    # maximum slope (change if monotonicity) of rho
        self.maxnuslope   = 2.    # and nu
        self.maxbetaslope = 2.    # linear (and 2nd, 3rd...) max slope of beta*
        ...
        self.rscale=[];        self.nu0rscale=[];     self.nu0pc=[]
        self.Rscale=[];        self.Nu0rscale=[];     self.Nu0pc=[]
        ...
        if self.investigate != 'walk' and self.investigate != 'triax' and self.investigate != 'gaia':
            ...
            self.Rscale.append(1.);           self.Rscale.append(1.)
            self.nu0rscale.append(1.);        self.nu0rscale.append(1.)
            self.Nu0rscale.append(1.);        self.Nu0rscale.append(1.)
